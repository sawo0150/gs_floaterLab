...
- 상태: **Phase 1+2 완료·채택, 실측 검증됨 (2026-07-22).** 평균 gaussian 개수 **−35.9%**,
  최종 개수 **−34~35%**, PSNR은 baseline과 동급(오차범위 내, kf PSNR은 오히려 소폭
  상승), evo APE(Sim3)도 baseline과 동급/개선 — **사용자가 이 loop에서 명시한 목표
  (평균 gaussian/frame 감소 → mapping 속도 향상)를 실측으로 달성.** Phase 3(carve
  loss 온라인 근사)도 구현·실행 완료(크래시 없음, PSNR −0.1~0.3dB 비용 확인)했으나
  **floater 개선 여부를 검증할 지표가 아직 없어 기본값 off로 유지**(아래 참조).
  Phase 2Q(품질 지향 스윕, Q1~Q4)는 이번 라운드에서 미실행 — Phase 2가 이미 같은
  방향(내용-적응 재배분)에서 강한 결과를 내 우선순위 밀림, 다음 라운드 후보.
...
### Phase 3 — Carve loss 온라인 근사 (구현·실행 완료, floater 지표 미검증이라 기본 off)
...
- **테스트 결과**(Phase 2 위에 `carve_lambda=0.05` 추가): 크래시 없이 정상 동작.
  평균/최종 gaussian 수는 거의 무변화(61,043/85,439 — carve는 opacity를 누르지
  개수를 직접 바꾸지 않으므로 예상대로), 시간도 무변화(59.40s), **PSNR은
  22.65/23.02로 Phase 2 단독(22.77/23.30) 대비 −0.12~−0.28dB 비용**, evo APE
  Sim3도 2.40cm로 소폭 상승(baseline과 동급, 여전히 ORB 대비 5배 이상 여유).
- **왜 기본값 off로 유지하는가**: PSNR 비용은 확인했지만 **floater가 실제로
  줄었는지 판단할 지표가 없다** — 이 시퀀스엔 region GT(exp43 방식)가 아직
  준비돼 있지 않음. "PSNR을 깎아가며 floater를 줄이는 트레이드오프"인지 "그냥
  손해"인지 구분이 안 되는 상태로 기본 채택하는 건 근거 없는 결정 — 계획에서
  명시했던 "region GT를 incremental 결과물에 적용 가능한지부터 확인"이 여전히
  Phase 3 완결의 진짜 다음 단계.
...
Training:
  carve_lambda: 0.0            # Phase 3 구현됨, floater 지표 없어 기본 off
  carve_margin: 0.05
```
...
1. **floater 품질 지표부터** — Phase 3(carve_lambda>0)의 net benefit을 판단하려면
   region GT를 이 incremental 결과물에 적용할 수 있는지 확인이 선행 과제.
2. Phase 2Q(Q1~Q4, 품질 지향 스윕) — 이번 라운드에서 미실행. Phase 2가 이미
   "적응적 재배분"으로 강한 결과를 냈으니, Q1(예산 상향)을 Phase 2의 적응형
   베이스 위에 얹어보면(현재 256을 더 낮춰 128 근처로) PSNR을 baseline보다
   실제로 끌어올릴 수 있는지가 다음 확인 지점.
3. Phase 1 캘리브레이션을 더 정밀하게(로컬 영역 독립 학습, 여러 반복으로 노이즈
   평균화)할 가치가 있는지는 Phase 2가 이미 근사치만으로도 강하게 통했으므로
   우선순위 낮음 — ROI 대비 낮은 항목으로 보류.
4. `growth_allowance`(현재 2.0 고정) 자체도 미탐색 축 — 더 낮추면(예: 1.3) 평균
