...
- 상태: **Phase 1+2+3 전부 완료·채택, 전부 실측 검증됨 (2026-07-22).** 평균 gaussian
  ...
  두 효과의 합성 결과 — 상세는 하단 "부록" 절. **⚠ 추가(2026-07-25)**: 남는
  실시간 예산(5.3s)을 `map()` 반복 횟수(iters 10→15/20)에 재투자하는 시도는
  **기각** — 큐 드롭 정책 때문에 반복을 늘릴수록 처리되는 keyframe 수만
  줄어(22→19→16회) PSNR이 개선되지 않거나(15) 오히려 예산을 초과(20)함.
  `iters=10` 유지, 다음 후보는 `queue_size` 확대. 상세는 하단 "mapping iters
  상향 테스트" 절.
...
### mapping iters 상향 테스트 — 부정적 결과 (2026-07-25, 사용자 질문 대응)

사용자 질문: "실시간이 되긴 하는데 품질이 썩 좋지는 않다... tracking돌리면서
mapping하면 mapping을 몇 iter 정도 할 수 있는거지?" — 현재 채택 레시피가 예산
(65.1s) 대비 **5.3s 여유**(59.8s)를 남기고 있으니, 이 여유를 keyframe당 `map()`
반복 횟수(`iters`, 현재 10)에 재투자하면 품질이 오르는지 직접 테스트.

- `gs_backend.py`의 정규 keyframe `map()` 호출(`iters=10`)을 `15`, `20`으로
  올려 각각 1253 전체 재실행(그 외 exp55 최종 레시피는 그대로: adaptive_density
  on, carve_lambda=0.05, ppm on).

| iters | 온라인 루프 총합 | 실시간 배수 | PSNR(mean/kf) | `map()` 성사 횟수 |
|---|---:|---:|---:|---:|
| **10(현재 채택)** | 59.8s | 0.92배 | **22.61 / 22.95** | 22회 |
| 15 | 62.53s | 0.96배 | 22.36 / 22.66(악화) | 19회 |
| 20 | 65.91s | **1.01배(예산 초과)** | 22.49 / 23.02(뒤섞임) | 16회 |

evo APE(Sim3)는 iters=15에서 2.41cm로 baseline과 동일 — 궤적 정확도엔 영향 없음
(iters=20은 미확인, 예산 초과로 이미 기각되어 추가 확인 불필요 판단).

**원인 — `_gs_queue` 드롭 정책과 정확히 맞물림**(위 부록에서 이미 확인한 그
메커니즘): `vigs.py::call_gs()`는 큐가 가득 차면 오래된 패킷부터 버린다
(`while self._gs_queue.full(): ... get_nowait()`). `iters`를 올리면 `map()`
호출 1회가 더 오래 걸려 mapper가 더 뒤처지고, 그만큼 **더 많은 keyframe이
아예 매핑을 못 받고 버려진다**(22→19→16회) — "한 keyframe에 더 많이 투자"가
"더 적은 keyframe만 커버"로 상쇄되고도 남는다. PSNR이 안 오르거나(15) 뒤섞이는
(20) 이유가 여기 있음 — **keyframe 커버리지(coverage)가 반복 깊이(depth)보다
지배적**이라는 뜻으로 해석.

**결론 — 기각, `iters=10` 유지.** "남는 실시간 예산 = 공짜 반복 횟수"라는
naive한 가정이 이 아키텍처(큐 드롭 정책)에서는 성립하지 않음을 실측으로 반증.
이 결과는 큐 드롭 자체를 줄이는 레버(`queue_size` 확대 등, 위 부록의 "다음
조사 후보" 1번)가 `iters` 상향보다 우선순위가 높다는 근거를 추가로 보강함 —
coverage를 지키면서 품질을 올리려면 "한 번 더 깊게"가 아니라 "더 자주/더 많이
받기" 방향이 맞을 가능성이 큼.

## 다음 단계

1. **`Training.queue_size`(현재 2) 확대 실험** — 위 iters 테스트가 "coverage가
   depth보다 지배적"임을 보여줬으므로, iters를 올리는 대신 큐를 키워 드롭되는
   keyframe 자체를 줄이는 쪽이 다음 우선 후보(신규, 2026-07-25 추가).
2. **carve floater 지표의 run-to-run 노이즈 폭 확인** — 지금은 carve-on/off 각각
   단일 비교라 "−7.5%"가 이 지표 고유의 노이즈 대비 얼마나 큰지 모름. 동일
   설정으로 2~3회 반복 실행해 노이즈 폭을 재면 이번 결과의 확신도가 올라감.
3. `carve_lambda`/`carve_margin` 스윕 — 0.05는 첫 시도값이었는데 비용이
   거의 없었다는 건 여유가 있다는 뜻일 수 있음(예: 0.1~0.2로 올려도 PSNR
   손실 없이 floater를 더 줄일 여지가 있는지).
4. Phase 2Q(Q1~Q4, 품질 지향 스윕) — 이번 라운드에서 미실행. Phase 2가 이미
   "적응적 재배분"으로 강한 결과를 냈으니, Q1(예산 상향)을 Phase 2의 적응형
   베이스 위에 얹어보면(현재 256을 더 낮춰 128 근처로) PSNR을 baseline보다
   실제로 끌어올릴 수 있는지가 다음 확인 지점.
5. Phase 1 캘리브레이션을 더 정밀하게(로컬 영역 독립 학습, 여러 반복으로 노이즈
   평균화)할 가치가 있는지는 Phase 2가 이미 근사치만으로도 강하게 통했으므로
   우선순위 낮음 — ROI 대비 낮은 항목으로 보류.
6. `growth_allowance`(현재 2.0 고정) 자체도 미탐색 축 — 더 낮추면(예: 1.3) 평균
   gaussian이 더 줄지만 densify의 국소 보정 여력이 줄어 품질 손실 가능, 스윕 여지.
7. `exp55_score_carve_vigs.py`(신규 재사용 가능 도구)를 다른 incremental 씬/실험
