...
run_branch () {
    ...
    local task_output="$task_root/${task_scene}_shared_loss_${task_label}_K32_r4_45k_s0"
    ...
    "$task_python" train.py \
        ...
        --view_scheduler staged_loss_interval_size_softmax_rr \
        --scheduler_seed 0 --scheduler_beta "$task_beta" \
        --scheduler_loss_alpha "$task_alpha" --scheduler_block_size 32 \
...
for task_scene in aria305 aria12F; do
    ...
    run_branch "$task_scene" b002_a05 0.02 0.5 "$task_checkpoint"
    run_branch "$task_scene" b002_a1 0.02 1.0 "$task_checkpoint"
    run_branch "$task_scene" b005_a05 0.05 0.5 "$task_checkpoint"
done
