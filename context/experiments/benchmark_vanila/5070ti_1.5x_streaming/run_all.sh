#!/usr/bin/env bash
# Resume-safe serial queue for all locally prepared UTMM and RPNG scenes.
set -uo pipefail
...
for task in "${tasks[@]}"; do
    ...
    output_dir="$lab_root/results/benchmarks/benchmark_vanila/5070ti_1.5x_streaming/$family/$scene/origin22ffe24_pure_online_stream15x_seed0"
    result_file="$output_dir/psnr/after_opt/final_result.json"
    contract_file="$output_dir/stream_contract.json"
    if [ -f "$result_file" ] && [ -f "$contract_file" ]; then
