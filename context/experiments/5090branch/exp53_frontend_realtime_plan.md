...
- 상태: **완료 (2026-07-22). 축A·B·C 채택. 축D는 재학습 없이는 구현 불가로 판정(실행 안 함). 축E(커널 튜닝)는 이미 목표 달성해 불필요. exp54와 합쳐 실시간 배수 1.52배→0.94배(실시간 돌파) 달성.**
...
## 축B·C 실행 결과 (2026-07-22, 같은 `/loop` 세션 — exp54 축1 위에서 스캔)

축A(1/0)+exp54 축1(`pcd_downsample=128`) 조합(72.91s, 1.12배)을 새 기준선으로 축B·C를
순서대로 추가:

| 축 | 설정 | 온라인 루프 총합 | 실시간 배수 | PSNR(mean/kf) | evo APE Sim3 RMSE | 판정 |
|---|---|---:|---:|---:|---:|---|
| 기준(축A+exp54축1) | thresh=2.4, window=25/radius=2 | 72.91s | 1.12배 | 22.39 / 22.72 | 1.59cm | 출발점 |
| **B** | `motion_filter.thresh` 2.4→3.6 | **61.67s (−15.4%)** | **0.947배 — 실시간 돌파** | 22.80 / 23.27 (오히려 상승) | 1.82cm | **채택** — 이 세션 두 번째 단일 축 최대 레버, keyframe 발생 자체를 줄여 tracking·mapping 양쪽 작업량을 동시에 낮춤 |
| **B+C** | + `frontend_window` 25→15, `frontend_radius` 2→1 | 60.65s (−1.7% 추가) | 0.931배 | 22.82 / 23.39 | 1.93cm | **채택** — 추가 이득은 작지만 순수 이득, 부작용 없음 |

**축B(`motion_filter.thresh`) 채택 — 이 라운드 최대 발견**: keyframe 판정 임계값을
올려 keyframe 발생률 자체를 낮춤. exp52에서 "fps를 낮춰도 keyframe 개수가 거의
안 준다"고 확인했던 것과 달리, **임계값 직접 상향은 keyframe 수를 실제로 줄여**
tracking과 mapping 양쪽 작업량을 동시에 낮추는 효과를 냄(축A는 tracking만,
exp54 축1은 mapping만 줄였던 것과 달리 **양쪽에 동시에 걸리는 레버**라 효과가 큼).
evo APE(Sim3)는 1.59→1.82cm로 소폭 상승했지만 ORB 기준(13cm) 대비 여전히 7배
이상 여유. **이 지점에서 처음으로 실시간(1.0배 미만) 돌파.**

**축C(`frontend_window`/`frontend_radius`) 채택**: 로컬 BA 그래프 크기를 줄여
`bundle_adjust`/`corr_lookup` 비용을 낮춤. 축B만큼 크진 않지만(−1.7%) 순이득이라
채택. `frontend_nms`는 이미 1(최소값)이라 추가로 건드릴 여지 없음.

**exp54(축4~7)와의 최종 통합** — 상세는 [exp54](exp54_gsmapping_speed_ablation_plan.md)
참조. 축A+B+C(frontend)에 exp54 축1(`pcd_downsample=128`)+축7(PPM)을 더한 **최종
레시피가 61.34s(0.94배)** — 5070 Ti 단일 GPU에서 실시간 최초 달성.

## 축D 조사 결과 — 구현 불가로 판정(실행 안 함)

`vigs/modules/corr.py`의 `CorrBlock`(`num_levels=4`, `radius=3`)은 `factor_graph.py`/
`droid_net.py`의 **사전학습된 ConvGRU(update_module) 호출부에 하드코딩**되어 있고,
correlation lookup 출력 채널 수(`num_levels × (2×radius+1)²`)가 그 **동결된 가중치의
입력 채널 수와 고정 결합**되어 있다 — 추론 시점에 radius/num_levels를 낮추면 텐서
shape가 학습된 conv 가중치와 안 맞아 크래시하거나 무의미한 결과가 됨. **재학습 없이는
손댈 수 없는 축**으로 확정(원안에서도 이미 "최소 기대효과+최고 위험"으로 최하위
우선순위였던 예상이 실측 조사로 확인됨).

## 남은 것 — 축E(커널 튜닝, 미착수)

`bundle_adjust`(CUDA BA solve) 커널 자체 최적화 — 고비용·고위험 엔지니어링이라
원안부터 최후순위였고, 이미 실시간(0.94배)을 달성해 지금은 불필요. 더 느린 GPU로
포팅할 일이 생기면 다음 후보.

## ①②③ dense correspondence 계측 (미착수)

축A~C 결과가 이미 결정적으로 좋아(정확도 저하 없이 실시간 달성) 사전 계획했던
①confidence weight ②iteration별 delta 수렴 ③BA 재투영 오차 계측은 결국 필요하지
않았음 — "왜" 정확도가 유지되는지 사후 설명이 필요해지면 그때 추가.
