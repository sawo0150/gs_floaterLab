...
- 상태: **305호 추가 검증에서 품질·균등성·admission 모두 악화 / 공동 방법론 NO-GO 강화**
...
- `/home/intern/VIGS-SLAM-main-integration-20260828/exp72_axes/run_scheduler_ablation.sh`
- `/home/intern/VIGS-SLAM-main-integration-20260828/exp72_axes/test_entropy_count_scheduler.py`
...
- seed 0
- scene: `aria1253`, `aria1253rot`, `aria301_305`
...
동일 RTX 5090 original-timestamp 1× 조건에서 `aria301_305` 전체 2,688 frame을
fresh paired run으로 검증했다.

| 지표 | final-v7 baseline | \(K=128,\beta=0.02\) | 변화 |
|---|---:|---:|---:|
| held-out PSNR | 28.934 dB | 28.558 dB | **−0.376 dB** |
| SSIM | 0.90407 | 0.90104 | −0.00303 |
| LPIPS | 0.24833 | 0.24892 | +0.00058 |
| online wall | 134.356 s | 134.361 s | +0.004% |
| streaming replay updates | 14,246 | 10,859 | **−23.8%** |
| final admitted pool | 674 | 528 | **−21.7%** |
| selection CV | 0.470 | 0.643 | **+36.9%** |
| middle/first lifetime count | 0.774 | 0.592 | 악화 |
| last/first lifetime count | 0.263 | 0.161 | 악화 |
| conditional entropy ratio | — | 0.99690 | 높은 값 유지 |

305호에서는 `−0.2 dB` 품질 보존 기준을 넘어서 회귀했다. 더 중요한 것은 동일 wall
budget에서도 후보의 streaming replay와 admitted pool이 함께 약 24%/22% 감소했다는
점이다. 후보가 active bonus 없이 거대한 전체 pool에서 거의 균등하게 섞기 때문에 새 view의
초기 2회 service가 늦어지고, 기존 global minimum maturity gate가 admission을 더 오래
막는다. 따라서 높은 **한-step 조건부 entropy**만으로 최종 frame exposure가 균등하거나
GPU 처리량에 비례해 admission된다고 볼 수 없다.

## 해석

### 확인된 것

- **두 짧은 trajectory의 품질 보존:** (K=128,\beta=0.02)는 aria1253/rot에서
  −0.2 dB 기준을 통과했고, 세 장면 모두 conditional entropy가 uniform maximum의
  99.69% 이상이었다.
...
- 305호에서는 높은 conditional entropy에도 selection CV가 0.470→0.643,
  middle/first가 0.774→0.592로 악화했다. 현재 entropy telemetry는 매 draw의 분포가
  평평하다는 뜻일 뿐, 시간 전체의 균등 exposure나 admission 안정성을 대신하지 못한다.

## 판정

- **opt-in 구현만 유지:** (K=128,\beta=0.02)는 분석·ablation 재사용용이며,
  305호에서 품질 보존 후보 지위도 상실했다.
...
- `/home/intern/VIGS-SLAM-main-integration-20260828/exp72_axes/results/exp72_scheduler_ablation_runs.csv`
- `/home/intern/VIGS-SLAM-main-integration-20260828/exp72_axes/results/exp72_scheduler_ablation_summary.json`
