...
### 축 5 — birth-redirect(신생아→표면 스냅): 소폭 positive (먼지 -8%)

| 12F | PSNR | free 먼지(원거리) |
|---|---:|---|
| hybrid+carve (기준) | 35.071 | 243 (239) |
| **축5 birth-redirect** | 35.103 | **224 (217)** |

- 먼지 243→224(-8%), PSNR 무손실. 신생아를 표면 anchor로 스냅 = geometry측 소폭 개선. 축3(-21%)보다 약함.

## 🏁 배치 종합 (8축, 07-15) — init이 단일 지배 레버 확정

| 축 | 개입 위치 | 12F/305 결과 | 판정 |
|---|---|---|---|
| **1 305 hybrid** | init | 305 PSNR **+1.33dB**·먼지 461→**4** | ✅ 대성공 |
| **2/2b 12F hybrid** | init | 12F **+3.0dB**(청소 후 유지)·먼지 1289→243 | ✅ 대성공 |
| **3 표면-확신 opacity** | init | 12F 먼지 **-21%**·PSNR 유지 | ✅ 성공 |
| 5 birth-redirect | geometry(출생) | 12F 먼지 -8%·PSNR 유지 | 🔸 소폭 |
| 7 원거리 감쇠(soft) | loss | 먼지↑·PSNR -0.54 | ❌ 기각 |
| 7b max-dist(hard) | loss | 무효(243→236) | ❌ 무효 |
| B footprint carve | carve | 먼지 ×5(243→1208) | ❌ 역효과 |
| 6 no-densify | densify | PSNR -1.32 | ❌ 역효과 |
| A budget 122k | init 경량화 | +3dB 소실(→baseline)·먼지 1 | ⚠️ 품질↔무게 트레이드 |

**결론: floater의 레버는 init에 있다.**
- **init을 만지는 축(1·2·3)은 전부 성공**(품질·청정), **loss/carve/densify를 만지는 축(7·7b·B·6)은 전부 실패/역효과.** geometry측 출생 개입(5)은 소폭. 8축 실험이 "압력이 아니라 init/geometry가 답"을 반복 확증.
- **실용 미결**: dense init(586k)이 +3dB의 근원인데 무거움 → 경량화(122k)하면 품질 소실. **중간 budget(250~350k) 탐색**이 분단위 파이프라인의 다음 관문.
- 사용자 원거리 통찰(round9)은 진단으론 옳았으나(먼지 98% 원거리), 처방(원거리 loss 제거)은 무효~역효과 — 원거리 먼지도 결국 init/geometry로 풀어야 함.
