...
> 마지막 갱신: 2026-09-11. 이 문서가 넘치면 내용을 `knowledge/` 또는 `rounds/`로 밀어낸다.
...
- **2026-09-11 — exp78 D1 strict mapping-only RPNG 3시퀀스 전이, 3/3 all-metric 승리.** clean official TensorRT tracker의 동일 frozen packet/pose/RGB를 두 mapper에 넣고 fixed 1.5×·사전고정 mapping-disjoint held-out·zero-tail로 비교했다. causal online-rank density+IMU rotation+appearance-only dense replay+auto-freeze gsSLAM은 table01 **25.363(+1.135)**, table06 **24.454(+1.377)**, 최초 untouched validation table02 **22.890(+2.282)dB**로 3/3 승리했고, 3-sequence mean은 **24.236/.8085/.2009**, vanilla **22.638/.7541/.2418** 대비 **+1.598dB/+.0543/-.0410**이다. 목표 +2dB에는 아직 0.402dB 부족하다. density1.5·FIFO first-service·pose-confidence weighting·endpoint hard admission·dense geometry gradient는 기각했고 hard carve는 사용하지 않았다. archive validator를 동일 검증 의미로 저메모리화하고 manifest-hash cache/pinned runner를 추가했다. 다음은 설정 변경 없이 RPNG 나머지 validation과 UTMM을 확장한다. → [exp78 D1](experiments/exp78/d_dataset_general_optimization/rpng_mapping_only_ablation.md)

- **2026-09-11 — exp78 A4 clean official VIGS all-16 + TensorRT 레인 종료(qualified public-artifact reproduction).** official commit `22ffe24` pre-final PyTorch all-16에서 valid table08 seed1을 쓴 RPNG 평균은 **22.585/.7488/.2385**로 논문 22.21/.723/.314 대비 PSNR `+.375dB`(허용폭 통과), UTMM은 **19.373/.6585/.4013**으로 논문 20.87/.687/.441 대비 PSNR `-1.497dB`, SSIM `-.0285`라 재현 실패다. 공개 repo에는 paper-state commit, UTMM mapping invocation, all-method non-KF union이 없어 exact UTMM state는 복원 불가함을 확정했다. README profile TensorRT 5개를 별도 root에 빌드·수치 검증(relative RMSE 최대 .300%, cosine≥.9999956)하고 fast/ego-drive/table06/table08에 적용한 결과 valid PyTorch 대비 PSNR 변화 최대 `.061dB`, ATE 변화 최대 `.0121cm`라 UTMM 격차 원인이 아님을 확정했다. 속도는 `1.064–1.254×`, allocated memory는 `0.86–1.63GiB` 감소. table08 TRT seed0는 과거 frame7930 붕괴를 통과해 논문과 정확한 **579KF**, ATE **3.401cm**, Recall100%, map-pose mean/max **2.00/28.60mm**로 완주했다. 다음은 공개 약한 UTMM 값을 paper baseline으로 오용하지 않고, 동일 frozen tracker packet으로 Lane-B mapping-only 비교를 구축한다. → [exp78 A4](experiments/exp78/a_paper_reproduction/vigs_all16_pytorch_reproduction.md)

- **2026-09-11 — exp78 A3 UTMM `ego-drive` 장시퀀스 반복도 paper parity 실패.** 동일 official clean pre-final seed0/1은 **20.3246/20.3772dB**(평균 20.3509, 논문 21.54 대비 -1.189), ATE **5.1759/5.2188cm**(평균 5.1974, 논문 1.45 대비 +3.747)로 seed 차이가 작다. map/IMU 시작은 frame54/131(3.86%/9.36%)로 fast의 48.49%/78.01%보다 빨라 PSNR 격차가 -3.960→-1.189dB로 줄었지만 tracking parity가 깨져 mapping 인과 해석은 불가하다. seed1 final BA+20 map update는 ATE를 3.328cm로 줄여도 논문 after-BA 2.13cm에 못 미치고, rendering은 **20.377→20.163dB(-.214)**로 오히려 악화했다. 따라서 hidden finalization으로 표를 복원하는 가설도 기각하고, 다음 품질 비교는 frozen packet/pose의 mapping-only 격리를 우선한다. → [exp78 A3](experiments/exp78/a_paper_reproduction/utmm_ego_drive_reproduction.md)

- **2026-09-11 — exp78 A2 UTMM 논문/공개 구현 protocol 불일치 원인 확정.** clean official `fast-straight` pre-final은 seed0/1/2 **18.0244/18.0311/18.0045dB**(평균 **18.0200**, range .0265)로 논문 21.98보다 3.960dB 낮다. 공식 in-process evaluator와 저장 PLY 재평가는 **0.00000025dB** 차이여서 evaluator/serialization을 배제했고, ATE .942cm·Recall100%, cached map pose 오차 .695mm/.00247°로 tracking/stale pose도 주원인이 아니다. visual BA+20 map update는 **+1.101dB**지만 여전히 -2.874dB이며 논문이 말한 pre-final 상태도 아니다. 공개 `eval_utmm_mono.py`는 `gsmapping=False`, evaluator는 mapping KF를 포함하고 all-method KF union은 미공개이며, 현재 `terminate()` 순서와 논문 설명도 모순이다. 따라서 exact UTMM paper invocation은 공개 artifact만으로 불충분하다는 source-level limitation을 확정하고, clean mapping-enabled pre-final을 별도 public-source 레인으로 전체 확장한다. 다음 frozen 판별은 더 긴 `ego-drive`로 short-sequence late-init 효과 확인. → [exp78 A2](experiments/exp78/a_paper_reproduction/utmm_protocol_diagnosis.md)

- **2026-09-11 — exp78 A clean official VIGS RPNG table_06 pre-final gate 통과(qualified).** 공식 commit `22ffe24`를 별도 clean clone/build하고 final BA·color refinement 전에 저장하는 runner/evaluator를 만들었다. seed0에서 **2,767 frames, 232 KF, 174,349 GS, 299.063s/9.252FPS**, Sim(3) ATE **1.5473cm**, Recall@10cm **100%**다. 공개 evaluator 의미 그대로는 **23.038/.7622/.2603**(742뷰, mapping KF 232개 포함), VIGS 자체 KF를 뺀 paper-compatible best effort는 **23.006/.7646/.2564**(510뷰)다. 논문 table_06 **23.47/.775/.304** 대비 PSNR·SSIM은 사전 허용폭 안이고 LPIPS는 오히려 0.044~0.048 낮아 대칭 허용폭 밖이다. 공개 소스가 `stride5 OR KF OR last`를 평가하고 KF 제외문을 주석 처리해, 논문의 “어느 방법의 KF도 아니며 mapping에도 미사용” split과 불일치함을 코드로 확정했다. 정확한 all-method KF union은 미공개라 exact split은 아직 복원 불가하다. 사전 고정 555 UID의 post-hoc 값 22.971은 KF 45개가 mapping에 들어가 strict 비교값으로 사용 금지. 첫 실행은 마지막 tensor IPC race로 결과 저장 전 실패했고 로그를 보존한 뒤 driver lifetime만 수정해 재실행 성공했다. 다음은 LPIPS/KF-count 차이와 seed 분산 확인 후 RPNG8+UTMM8로 Lane A를 확장한다. → [exp78 A](experiments/exp78/a_paper_reproduction/README.md)

- **2026-09-11 — exp77 E2 reserve20↔0 완료·RPNG TensorRT 준비.** 사용자 문제정의를 재확인했다: 1253/305에서 이미 나온 목표 품질을 새 RPNG/UTMM 데이터로 옮길 때 생긴 회귀의 최적화 지점을 찾는 단계다. table_06 20→0→0→20ms에서 held-out 평균 **20.998→21.426dB(+0.428)**, dense replay **1,873.5→2,105(+12.36%)**, Adam **2,244.5→2,485(+10.72%)**였고 ATE는 2.749→2.739cm로 동일했다. 4/4 zero-tail 통과. 따라서 실행 예산/학습 기회 병목은 확인됐지만 잔여 격차의 전부는 아니다. RPNG `344x616`용 fnet/update/PGBA TensorRT FP16 엔진을 RTX5090에서 빌드·수치 검증했고 기존 Aria 엔진을 보존하는 opt-in 경로를 추가했다. 다음 판별은 TensorRT on/off matched 비교 후 initialization·coverage 축이다. → [exp77 E](experiments/exp77/e_reserve_ablation/README.md)

- **2026-09-11 — exp77 E replay 병목 분석·E2 진행.** [분석](experiments/exp77/e_reserve_ablation/replay_analysis.md): E1의 reserve40→20에서 dense update1889→2073(+9.74%)에 그쳤다. 소비 프레임 기준 deadline이 backlog 동안 과거로 남고, packet 우선 dispatch·map 상태 gate가 idle replay를 추가 제한한다. polling reject 비율은 시간 비율이 아니다. 사용자 요청으로 exp77 E에서 수정·비교를 계속하며 E2는 table_06 reserve20↔0 각2회, 1×·EOS cutoff 유지다.

- **2026-09-11 — 사용자 번호 운영 지시(이전 새 exp78 지정 정정).** 현재 작업을 **[exp77 E](experiments/exp77/e_reserve_ablation/README.md)**로 이동했다. **사용자가 명시적으로 exp78로 넘어가라고 하기 전까지 현재·후속 작업 모두 exp77 하위 단계로 진행하며 exp78을 만들지 않는다.** 아래 과거 exp78 실행 시작 항목은 현재 exp77 E를 뜻한다. table_06 reserve 4회 비교 완료: 40ms 평균 **21.117 dB**, 20ms 평균 **21.391 dB**(+0.274), 4/4 optimizer zero-tail 통과. 원본 실행 ID/경로는 provenance를 위해 보존한다.

- **2026-09-11 — 새 exp78 A 실행 시작.** [exp78](experiments/exp78/README.md)에서 RPNG table_06만 reserve 40→20→20→40ms 순서로 비교 중이다. 기존 final-v7 clean·fixed1×·optimizer zero-tail·seed0를 유지하고, 프로파일 적용 후 reserve만 바꾸는 별도 runner를 사용한다. 첫 실행 로그에서 reserve=40과 terminal rematuration/prune=0을 확인했다. 원본 VIGS 소스 수정 없음. 결과 경로는 `results/experiments/exp78_reserve_ablation_20260911/`이며 아직 품질 결과는 미확정이다.

- **2026-09-11 — 사용자 요청에 따른 실제 번호·폴더 통합.** 이전 exp77~80을 [exp77 A~D](experiments/exp77/README.md) 아래로 이동했다. 이전 exp78→B, exp79→C, exp80→D이며, 아래 과거 항목은 당시 ID를 보존한다. 새 작업은 [exp78](experiments/exp78/README.md)의 소수 시퀀스 최적화 원인 검증으로 이어간다. 다음 비교는 table_06 reserve 40ms↔20ms이며 전체 16개 재실행은 하지 않는다.

- **2026-09-11 — 실험 번호별 기록을 목적별로 묶음(새 실행 없음).** [최근 실험 흐름](experiments/RECENT_TRACKS.md)에 exp69~70 지도 유지·loss, exp70-S~76 replay 정책, **exp77~80 final-v7 재현·종료 조건·벤치마크 전이**를 연결했다. exp77~80은 한 검증의 네 단계로 읽는다. 기존 ID·결과 경로는 유지하고, 동일 질문의 후속 설정/seed 반복은 기존 exp의 하위 단계로 기록한다. 다음 reserve 비교는 exp80 후속이며 아직 미실행이다.

- **2026-09-10 (exp80 — RPNG/UTMM fixed1× optimizer zero-tail 16시퀀스 종료·원인 분석)**:
  exp79 설정을 16개에 적용해 **13개 평가/3개 실패**를 기록했다. held-out 평균
  UTMM **15.794**, RPNG **18.986**, 전체 **17.268dB**, 27dB 통과 0개다.
  평가된 13개는 모두 producer deadline·sensor-EOS 뒤 completed Adam update 0회다.
  slow-straight-1은 빈 map, table07은 PGBA CUDA index assert, table08은 frontend
  correlation 확장 OOM으로 실패. square2는 평가됐지만 ATE 92.8cm/recall 2.74%다.
  **핵심 설정 문제 확인:** 5090 profile의 tracking reserve 40ms가 benchmark
  30fps/33ms 간격보다 커서 overlap gate 허용이 13개 모두 0회(Aria20fps는
  710/675회)다. 짧은 UTMM 초기화는 stream 73–76% 뒤에 시작한다.
  PSNR 인과효과 크기는 아직 미검증이며, exp77 대비 matched13 평균 −1.543dB는
  여러 설정 차이가 섞였다. **사용자 지시: 이후 전체16 반복 금지, Aria1253·table06·
  fast-straight 중심 소수 반복; 먼저 overlap reserve 단독 ablation.**
  → [exp80 카드](experiments/exp80/exp80_vigs_1x_zero_tail_16seq.md),
  [원인 분석](experiments/exp80/exp80_root_cause_analysis.md)

- **2026-09-10 (exp79 — Aria fixed 1× sensor-EOS optimizer zero-tail 검증)**:
  현재 final-v7에 opt-in 종료 가드를 넣고 terminal pose 재학습/pruning을 껐다.
  producer 기준 65.10초 예산, 50ms 안전 여유와 CUDA 완료 계측에서 마지막
  Adam 완료는 65.0497/65.0508초이며 마감·sensor-EOS 뒤 update는 **모두 0회**다.
  동일 fixed held-out 262장 PSNR **26.177/27.499dB, 평균 26.838dB**로
  optimizer 계약은 2/2 통과했으나 **안정적 27dB 달성은 실패(1/2)**다.
  per-step 동기화 계측 비용을 포함하며 전체 tracking/export/eval 지연 보장은 아니다.
  exp78 대비 −1.086dB는 tail 제거·pruning 해제·계측 차이가 섞여 단일 인과효과로
  해석하지 않는다. 기존 기본 실행 경로는 env opt-in 없으면 유지한다.
  → [exp79 카드](experiments/exp79/exp79_aria_1x_sensor_eos_zero_tail.md),
  [evidence](experiments/exp79/evidence/summary.json)

- **2026-09-10 (exp78 — 현재 코드 Aria final-v7 27dB 재현 2/2)**:
  exp72/73의 RTX5090 전용 1×·legacy carve off·terminal dust-GC/pose rematuration
  baseline을 현재 dirty 코드로 재현했다. aria1253 디렉터리의 1,303 frame,
  동일 YAML·동일 fixed held-out 262장에서 **27.987/27.862dB, 평균 27.924dB**다.
  과거 문서의 27.708은 mixed-eval 평균이며 실제 held-out 기준은 **27.646dB**로
  지표 해석을 정정한다. 종료 뒤 명시적 rematuration은 **904/40 update**이므로
  이번 재현은 final-map 성과이며 exp57 freeze800 strict zero-tail 재현은 아니다.
  exp77 runner는 5090 profile을 명시적으로 불러오지 않아 실행조건 비교가 필요하지만,
  이 차이만으로 데이터셋 간 품질 격차의 원인을 확정하지 않는다. 코드 변경 없이 완료.
  → [exp78 카드](experiments/exp78/exp78_aria_final_v7_reproduction.md),
  [evidence](experiments/exp78/evidence/summary.json)

- **2026-09-10 (exp77 — VIGS final-v7 RPNG/UT-MM 16-sequence baseline 완료)**:
  ETH-preprocessed RPNG 8개와 UT-MM 8개를 current dirty VIGS final-v7로 실행해
  **15/16**에서 fixed held-out 렌더·trajectory 지표를 얻었다. 성공 run 평균은 RPNG
  **19.770dB**, UT-MM **18.159dB**, 전체 **19.018dB**로 27dB에 크게 미달했다.
  UT-MM `slow-straight-1`은 13 KF로 init threshold 15에 못 미쳐 empty-map 실패했고,
  `square-2`는 완주했지만 ATE **179.3cm**/recall@10cm **1.37%**라 tracking metric
  failure다. RPNG `table_08`도 16.2cm/61.6%로 약하다. 로그·코드 감사 결과 마지막
  센서 frame 뒤 pending mapping queue를 drain하므로 최초 `zero_tail` 명칭을
  **`fixed1p5_prefinal_queue_drain_v1`**로 정정했다. 따라서 이는 VIGS-style pre-final
  baseline이며 프로젝트 strict zero-tail이나 논문 표 수치와 직접 비교할 결과가 아니다.
  → [exp77 카드](experiments/exp77/exp77_vigs_final_v7_rpng_utmm_16seq.md),
  [evidence](experiments/exp77/evidence/aggregate.json)

- **2026-09-10 (벤치마크 준비 — 정리 및 RPNG/UTMM 다운로드 완료, 실험 아님)**:
  사용자 승인으로 동일 GT 60,007개를 hash 검증 뒤 hardlink 공유하고 일반 중간 PLY
  106개 및 해제본과 내용 일치를 검증한 전송 archive 5개를 삭제해 **30.3957GiB**를
  확보했다. 최종 모델 158개와 보호 중간 모델 49개는 보존했고 링크 차이가 있는 archive
  2개는 남겼다. 현재 워크스페이스 `data/benchmarks/`에 ETH 전처리 RPNG 8개·UTMM
  8개 시퀀스를 다운로드·SHA256 기록·ZIP CRC 검증·압축 해제했다. VIGS 입력 경로 기준
  RGB 49,080장과 필수 IMU/GT 파일을 확인했다. 저장소 clone/설치와 benchmark 실행은
  사용자 지시에 따라 보류 상태이며, 새 품질 수치는 없다.
  → [정리 audit](research/intern_cleanup_applied_20260910/README.md),
  [준비 코드·경로](../benchmarks/online_gs/README.md),
  [dataset manifest](../benchmarks/online_gs/prepared_datasets.json)
...
- **2026-09-04 (exp73 추가 — \(\kappa=22\) 1253 역전이 2회 통과, 첫 2-scene 공동 후보)**:
  305호에서 품질을 회복한 gate-free token cost \(\kappa=22\)를 사용자 요청으로
  aria1253에 그대로 적용하고 동일 설정을 한 번 반복했다. held-out PSNR은
  **27.750/27.672dB, 평균 27.711dB**로 fresh maturity-gate baseline 2회 평균
  27.708dB 대비 **+0.003dB**였고, 두 run 모두 −0.2dB 기준을 통과했다. final pool은
  **275/276장, 평균 275.5장**으로 baseline 422.5장 대비 **−34.8%**였다. 따라서 바로
  아래의 “고정 token 가격 cross-scene 일반화 미해결”은 정정한다: \(\kappa=12\)의
  1253→305 전이는 실패했지만, **\(\kappa=22\)는 현재 1253(2회)과 305(1회) 모두에서
  품질을 보존한 첫 공통 engineering candidate**다. 다만 305 baseline을 보고 선택된 값이고
  두 장면뿐이므로 universal constant 증명은 아니다. 또한 selection CV는 1253 평균
  **0.951**(baseline 0.373), middle/first 0.655로 여전히 악화해 scheduling 문제는
  해결되지 않았다. gate-free run은 총 7개·526 admission poll이 되었고 token-law 정수
  오차는 계속 전부 0이다. **결론: 다음 scheduler 실험의 admission 기본값으로
  \(\kappa=22\)를 사용하되 production default는 보류한다.**
  → [exp73 갱신 카드](experiments/exp73/exp73_gate_free_token_admission_real_ablation.md),
  [갱신 evidence](experiments/exp73/evidence/exp73_gate_free_token_admission_summary.json)

- **2026-09-04 (exp73 — maturity gate 제거 + GPU-token admission 실제 1253/305 검증)**:
  final-v7의 global `minimum selection count >= 2` barrier를 opt-in으로 제거하고,
  최초 전역 seed 뒤 완료 dense Adam update \(\kappa\)회당 pending causal view 한 장을
  admission하는 \(A_{paid}(u)=\lfloor u/\kappa\rfloor\) controller를 구현했다. 5개 실제
  run의 406 admission poll에서 정수 오차는 전부 **0**이었다. aria1253 sweep은
  \(\kappa=8/12/16\) PSNR **25.668/27.644/22.163dB**였고, \(\kappa=12\)가 fresh
  baseline 2회 평균 27.708dB 대비 **−0.064dB**로 통과했다. 그러나 같은 \(\kappa=12\)를
  305호에 그대로 옮기면 pool 674→**1,215**, PSNR 28.934→**27.388dB(−1.546)**로
  실패했다. baseline pool을 보고 정한 진단값 \(\kappa=22\)는 pool 749,
  **28.815dB(−0.119)**로 회복했지만 범용 parameter 증거는 아니다. 또한 scheduler를
  active/archive 그대로 둔 결과 selection CV는 1253 0.373→0.895, 305
  0.470→0.942로 악화했다. **결론: maturity gate 없이 GPU-work 비례 pool 증가는
  정확히 가능하지만, 고정 token 가격의 cross-scene 일반화와 최종 count 균등성은
  해결되지 않았다. admission/scheduling 분리는 유지하되 production default는 NO-GO.**
...
- **2026-09-04 (exp72-305 — ERCB 선정 후보 305호 일반화 실패)**:
  `aria301_305` 2,688 frame에서 final-v7 baseline과 \(K=128,\beta=0.02\)를 RTX5090
  original-timestamp 1×·seed 0으로 fresh paired 검증했다. 후보는 conditional entropy
  ratio **0.99690**과 동일 online wall(134.356→134.361s)을 유지했지만, held-out PSNR이
  **28.934→28.558dB(−0.376)**로 품질 기준을 실패했다. streaming replay는
  **14,246→10,859(−23.8%)**, final admitted pool은 **674→528(−21.7%)**로 줄고,
  selection CV는 **0.470→0.643**, middle/first는 **0.774→0.592**로 악화했다.
  전체-pool high-entropy draw가 신규 view를 빨리 두 번 서비스하지 못해 기존 global-minimum
  maturity gate를 더 오래 닫은 결과다. **결론: 높은 one-step entropy는 최종 균등성이나
  pool-independent admission을 보장하지 않으며, \(K=128,\beta=0.02\)의 품질 보존 후보
  지위도 철회한다. 다음은 scheduler 추가 tuning이 아니라 GPU-token admission과 초기
  service 정책을 gate에서 분리해야 한다.**
  → [exp72 카드](experiments/exp72/exp72_entropy_count_scheduler_real_ablation.md),
  [305 evidence](experiments/exp72/evidence/exp72_305_scheduler_ablation_summary.json)

- **2026-09-04 (exp72 — ERCB statistical-block scheduler 실제 final-v7 A/B)**:
  exp71의 \(p(i\mid\mathcal R)\propto e^{-\beta n_i}\)를 실제 final-v7 replay draw에
  opt-in 이식하고 B1 \(\beta=0/0.006/0.02/0.05\), fixed statistical block
  \(K=32/128\), active-aware 변형을 RTX5090 original 1×에서 sweep했다. 첫 구현이
  `remaining`을 full epoch에 유지해 \(\beta\)가 lifetime count가 아닌 순서만 바꾸던
  오류를 교차장면에서 발견·폐기하고, simulator와 같은 block-reset Plackett–Luce law로
  수정해 기존 회귀 포함 60 tests를 통과했다. 최선 \(K=128,\beta=0.02\)는 aria1253
  **27.624dB**(fresh baseline 2회 평균 27.708, −0.084), aria1253rot
  **25.177dB**(baseline 24.814, +0.362), online wall 차이 ≤0.04%, conditional entropy
  ratio 0.9987/0.9984로 **품질 보존 engineering candidate**다. 그러나 middle/first
  lifetime count가 1253 0.971과 달리 rot에서 baseline 0.758→**0.520**으로 악화했고,
  최종 pool도 1253 baseline 422.5→333, rot 353→334로 달라졌다. \(\beta=0.05\)+active도
  rot 0.605/entropy 0.9825에 그쳤다. **결론: opt-in 후보는 유지하되 production default와
  논문 방법론으로는 NO-GO; maturity gate가 남아 pool-independent admission도 미달이다.**
  → [exp72 카드](experiments/exp72/exp72_entropy_count_scheduler_real_ablation.md)

- **2026-09-04 (exp71 — growing-pool joint admission/scheduling 문제 재정의)**:
  사용자가 요구한 세 일반 조건을 (1) 초기 cohort↔실제 \(T/2\) admission cohort의
  lifetime-count equality, (2) \(A(u)=\lfloor u/\kappa\rfloor\) GPU-token admission,
  (3) full-pool without-replacement shuffle entropy와 finite-population-normalized gradient
  MSE로 다시 정의했다. 기존 예비 metric이 final-pool midpoint를 사용해 느린 admission에서
  초기 32장끼리 비교하던 오류를 발견·수정했고, 그 오류에서 나온
  \(\kappa=816,\beta=0.006\) 추천은 철회했다. 수정 후 \(B=8/16/32\), 6k/12k/18k
  updates, \(\kappa=16\)~4096, \(\beta=0\)~\(\infty\) sweep에서 공동 acceptance를
  만족한 점은 **0개**였다. 빠른 admission은 equality 보정이 trajectory-gradient를
  편향시키고, 느린 admission은 pool이 batch에 비해 작아 causal least-count oracle조차
  halfway cohort를 따라잡지 못했다. 이는 constant positive admission + raw lifetime
  equality + full-pool uniform shuffle의 장기 exact incompatibility와 일치한다. 반면 token
  admission은 전 run에서 tracking error/budget violation=0이었고, maturity gate는
  \(B=32,T=18k,\kappa=32\)에서 목표 admission의 uniform 18.9%/active-bonus 28.4%만
  처리했다. **결론: gate-free token admission 원리는 유지, valid \(\beta\)는 없으며 다음은
  uniform objective+age-adjusted fairness / lifetime equality+weighted objective /
  bounded pool·tail 중 하나를 먼저 선택해야 한다.** 실제 VIGS/PSNR 검증은 미실행이다.
  → [exp71 카드](experiments/exp71/exp71_joint_admission_scheduler_problem.md),
  [전체 보고서](research/joint_admission_scheduler_sim_report.md)

- **2026-09-04 (exp70-S — growing-pool maximum-entropy scheduler 합성 검증)**:
  ...
  → [exp70-S 카드](experiments/exp70/exp70_max_entropy_view_scheduler_sim.md),
  [전체 보고서](research/view_scheduler_long_horizon_sim_report.md)

- **2026-09-02 (exp70 1× accounting 재정정 — 새 loss의 compute tax 가설 기각)**:
  바로 아래 final-v7 1× 항목의 “replay update −16.9/−13.5%”는 total count에 arm별
  EOS rematuration update를 섞은 계산 오류였다. total 7,326/6,089/6,340에서
  EOS 1,416/0/384를 빼면 실제 streaming replay는 control/alpha/NLL
  **5,910/6,089/5,956(+3.0/+0.8%)**이다. 전체 loop에서도 EOS wall 4.567/0.000/1.197초를
  빼면 streaming phase는 **69.064/69.098/69.066초**로 사실상 동일하다. detector는
  226–227회 호출, 별도 microbenchmark 1.17ms/call이므로 이 실행에서 mapping budget을
  유의미하게 뺏었다는 근거가 없다. 따라서 PSNR 28.148→27.706/27.792, nominal floater
  148→161/164, giant splat 증가는 compute tax로 설명하면 안 된다. **final-map recipe
  NO-GO 판정은 유지**하되, 원인 분리는 EOS rematuration off + 동일 online update 수·view
  순서의 fixed-schedule A/B가 필요하다. 이 항목이 바로 아래 기록의 compute/scheduling-tax
  해석과 “detector fuse가 다음 우선순위” 결론을 대체한다.
  → [exp70 corrected result](experiments/exp70/exp70_score_free_detached_termination_loss.html)

- **2026-09-02 (exp70 최종 교정 — final-v7 RTX 5090 original 1×에서 legacy carve를
  대체한 alpha/NLL 통합 NO-GO)**: 앞선 exp70이 final-v7이 아니라 exp69 baseline의
  1.5×였음을 사용자 질문으로 재확인하고, final-v7 5090 hardware profile
  (original timestamp 약 20FPS, replay iters4/B1/dedicated stream/overlap)을 적용하는
  `run_v7_1x_no_legacy_carve.sh`를 추가했다. 기존 causal-carve는 λ=0만 둔 것이 아니라
  flag 자체를 생략해 세 arm 모두 `CAUSAL_CARVE_DONE enabled=False`를 확인했고, final-v7의
  terminal dust-GC와 EOS pose rematuration은 유지했다. aria1253 seed0 결과
  control/alpha/NLL은 held-out **28.148/27.706/27.792dB**, SSIM
  .88466/.87662/.87608, LPIPS .21726/.22720/.22868이었다. manual-region op&gt;.3은
  eroded **85/83/85**, nominal **148/161/164**, dilated **383/401/407**로 alpha는
  core 2개만 줄이고 넓은 두 mask가 악화, NLL도 개선 없음. control-gauge scale&gt;1m가
  **1→206/56**, opacity-area가 **218.78→1585.56/1270.27**로 giant splat도 증가했다.
  detector 선택률은 두 arm 모두 2.691%였지만 1× replay update가 control 7,326 대비
  6,089/6,340(−16.9/−13.5%)으로 줄어 penalty와 scheduling tax가 결합됐다. 따라서
  **현재 final-v7 1× whole-system recipe는 NO-GO**이며 다음은 loss 강도 sweep이 아니라
  detector를 기존 depth-loss 계산에 fuse해 update budget을 보존한 fixed-schedule A/B다.
  EOS rematuration이 있어 이 교정은 final-map 비교이지 strict zero-tail 주장은 아니다.
  detector 10개+causal-carve 33개 회귀 test는 재통과했다.
  → [exp70 corrected result](experiments/exp70/exp70_score_free_detached_termination_loss.html)

- **2026-09-02 (exp70 추가 정정 — FastMRI manual-region GT 복구, direct alpha의 단일-seed
  floater 감소 확인; NLL 우월성 기각)**: FastMRI desktop의 ORB-frame
  `1253_region_gt.npz`와 ORB keyframe trajectory를 5090에 복구하고, exp68의 trajectory
  Umeyama Sim3 evaluator를 경로 독립형으로 이식했다. control/alpha/NLL 정합 median은
  2.71/2.95/2.17cm(p90 4.42/4.89/3.73cm)로 region 비교 가능했다. direct alpha는
  control 대비 op>0.3 floater를 eroded/nominal/dilated mask에서 **83→73(−12.0%),
  174→148(−14.9%), 427→388(−9.1%)**로 모두 줄였고, nominal opacity-support와
  opacity-area-support도 −11.5/−6.7%로 일관 개선했다. NLL은 nominal/dilated visible은
  −10.3/−5.6%였지만 eroded core가 +16.9%, nominal opacity-area-support가 +2.3%라
  direct alpha보다 낫지 않았다. detector-only는 alignment median 1.06m로 붕괴해 region
  해석에서 제외했다. **판정: direct alpha는 27.312dB·시간 무비용과 함께 단일-seed
  floater efficacy positive, NLL 우월성은 기각. 범용 PASS는 반복 seed·교차장면 GT 전까지
  보류.** → [exp70 result](experiments/exp70/exp70_score_free_detached_termination_loss.html)

- **2026-09-02 (exp70 — score-free detached opacity loss 구현·strict 예비 검증, geometry 판정 보류)**:
  기존 `transit/(transit+terminal+e)` persistent score를 대체할 최소 방법으로,
  current depth-KF의 unoccluded Gaussian ray-density mixture에서 terminal depth가
  지지하는 surface mode와 그 앞의 마지막 prominent valley를 찾고, valley 앞에
  support mass 95% 이상이 놓인 Gaussian만 일회성 mask로 선택하는 detector를
  VIGS-SLAM main-integration에 opt-in 구현했다. action은 renderer `T_i`나 xyz force 없이
  primitive opacity logit에 `softplus(theta)=-log(1-opacity)`를 적용하며, optimizer
  step마다 depth KF 하나만 round-robin으로 검사한다. persistent Gaussian/voxel score와
  hard prune는 없다. 합성 detector·정확 gradient·backend opacity-only hook 10개와 기존
  causal-carve 33개 회귀 test가 모두 통과했다. aria1253 1303-frame strict 1.5× zero-tail
  단일-seed 4-arm 결과는 control **27.013dB**, detector λ=0 **25.604dB**, direct-alpha
  λ=.002 **27.312dB**, NLL λ=.002 **27.229dB**였고 loop 시간은 각각
  100.416/100.503/100.431/100.447초였다. alpha/NLL은 projected Gaussian의
  2.678/2.640%만 선택했고, control gauge 정렬 global opacity-area가
  513.27→379.97/346.03, scale>1m가 27→27/18로 악화 없이 줄었다. 다만 λ=0 arm은
  미세한 GPU timing 차이 뒤 tracking/PGBA 경로가 갈라져 Sim3 scale .2506,
  trajectory median residual 1.25m로 붕괴했다. 따라서 단일 run의 dB 차이로 loss의
  순수 효과나 NLL>alpha를 주장할 수 없고, 이 머신에는 aria1253 manual-region mask도
  없어 global PLY 변화만으로 floater PASS를 내리지 않는다. **판정: 구현·27dB/시간
  feasibility는 PASS, floater 효과·NLL 우월성은 OPEN.** 기본 동작은 off이며 다음은
  동일 schedule 고정 반복 A/B와 region GT asset 복구다.
  → [exp70 loss/result](experiments/exp70/exp70_score_free_detached_termination_loss.html)
