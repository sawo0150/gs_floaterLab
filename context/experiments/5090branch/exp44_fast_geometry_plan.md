...
- **45a 노출 모델링: 기각** — 공정 측정(노출 적용 전체이미지 PSNR) **27.79** (-6dB). `train_test_exp` 모드가 eval-split 전용 설계(테스트뷰 반쪽 마스킹·노출 최적화 프로토콜)라 순수 train 유스케이스와 부정합 추정. 재도전하려면 전체이미지 학습 + 노출항만 추가하는 별도 통합 필요.
- 44e2@30k: 33.412 — init 대전 최종 순위 변동 없음 (하이브리드 33.80 > PPM 33.52 > GI-net 33.41 > RoMA 33.01).
- **45b dynamic carve 구현·발사**: 가시 gaussian 점유를 terminal 증거에 주기 가산 → rho 동적 재계산 (`dynamic_carve: true`). feature-poor 표면 오분류의 자동 완화 기대.

### 45b dynamic carve — 신기록 경신 (07-13)

**train PSNR 33.883 / region 먼지 192 / 가시 63** — 챔피언(44d2: 33.799/234)을 PSNR·먼지 양쪽에서 경신.
메커니즘: 가시 gaussian 점유의 terminal 되먹임 → 재구성이 확정한 표면의 rho 하락 → ① feature-poor 표면 오분류 완화(PSNR↑) ② score가 진짜 빈 공간에 집중(먼지↓). **최종 레시피에 dynamic_carve: true 정식 편입 후보** — held-out 확인 중.

### 45b held-out + 44e3 발사 (07-13 새벽)

- **45b dynamic carve held-out: 32.378** — 챔피언(32.479)과 동급(노이즈 내). 판정: **train +0.08·먼지 -18% 무손실 → dynamic_carve 옵션 채택.**
- **44e3 발사**: PPM init + 구조텐서 이방성 per-point scale/rot 주입 (CARVE_INIT_ATTRS 경로, gaussian_model 확장). 비교선 44f2(33.524) — attrs 효과 격리.

## 45c: progressive resolution 2단계 — ❌ 기각 (07-13 아침)

| | 45c (r/2 8k → r1 재개 15k) | 44d fast 기준 |
|---|---|---|
| 총 시간 | **47분** (stage1 22 + stage2 25) | **8분** |
| PSNR@15k | 32.485 | 32.347 (+0.14 = 노이즈 이내) |
| region 먼지 | **343** | **147** |

- **시간 완패**: 2단계 = 데이터셋 2회 로드 + r/2 리사이즈 비용이 학습 단축분을 압도. 단일 프로세스 내 해상도 전환이 아니면 구조적으로 구제 불능.
- **"저해상도 densify가 먼지↓" 가설 기각**: 오히려 2.3배 악화 — 저해상도에서 gradient·carve 신호가 뭉개져 출생 판단이 나빠지는 것으로 해석.
- 부산물: checkpoint-resume 경로의 계측·계보 버퍼 크기 불일치 버그 2종 발견·수정 (train.py restore 직후 8종 버퍼 재생성). exp45 큐 4종 전부 판정 완료 (45a 기각 / 45b 조건부 / 44e3 보류 / 45c 기각).
