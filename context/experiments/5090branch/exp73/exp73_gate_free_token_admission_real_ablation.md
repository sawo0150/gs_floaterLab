# exp73 — Maturity gate 제거 + GPU-token admission 실제 장면 검증
...
줄었다. 반면 중간값인 \(\kappa=16\)은 22.163 dB로 무너졌다. 따라서 실제 online 3DGS의
...
1. **Maturity gate는 GPU-work-proportional admission에 필요하지 않다.** Gate 없이도
   \(A(u)=\lfloor u/\kappa\rfloor\)를 실제 실행에서 정확히 만족했다.
...
3. **Gate 제거만으로 최종 학습 균등성은 좋아지지 않았다.** 선택 scheduler를 그대로 둔
