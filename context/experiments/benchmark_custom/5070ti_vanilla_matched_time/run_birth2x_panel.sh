#!/usr/bin/env bash
# exp86-C: isolate 2x causal keyframe birth density under the adopted r4 contract.
set -euo pipefail
...
export MAPPING_BIRTH_DOWNSAMPLE_MULTIPLIER=0.5

for scene in fast-straight ego-centric-1; do
    for selector in rr ercb_view_count_b002_k128; do
        output="$results_root/$scene/${selector}_unified_pool_workcredit_cycle_r4_birth2x_seed0"
        echo "EXP86_C_RUN scene=$scene selector=$selector output=$output"
