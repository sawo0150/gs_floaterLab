#!/usr/bin/env bash
# Strict fixed-arrival runner with a causal per-mapping-packet Adam budget.
set -euo pipefail
...
output=${6:-"$lab_root/results/ERCB_ablation/exp03-E_packet_budget_interaction/$family/$scene/q${quota}/${selector}_arrival_s${seed}"}
