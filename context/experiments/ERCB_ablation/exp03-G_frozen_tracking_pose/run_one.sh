...
output=${6:-"$lab_root/results/ERCB_ablation/exp03-G_frozen_tracking_pose/$family/$scene/q${quota}/${selector}_s${seed}"}

case "$family" in
    utmm)
        gt_trajectory="$lab_root/data/benchmarks/utmm/prepared/UTMM_Dataset/$scene/groundtruth.txt"
        gt_camera_frame=utmm_robot
        ;;
    rpng)
        gt_trajectory="$lab_root/data/benchmarks/rpng/prepared/rpngar/$scene/gt.txt"
        gt_camera_frame=camera
        ;;
...
export MAPPING_GT_TRAJECTORY="$gt_trajectory"
export MAPPING_GT_CAMERA_FRAME="$gt_camera_frame"
