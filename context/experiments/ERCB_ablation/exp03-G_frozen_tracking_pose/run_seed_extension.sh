...
root="$lab_root/results/ERCB_ablation/exp03-G_frozen_tracking_pose"
...
for seed in 1 2; do
    for spec in "rpng table_07 3" "utmm square-1 15"; do
        ...
        for selector in rr_role_stratified ercb_relative_floor_role_stratified; do
            ...
            output="$root/$family/$scene/q${quota}/${short}_s${seed}"
            result="$output/psnr/online_final/final_result.json"
            audit="$output/sensor_eos_audit.json"
            if [ -f "$result" ] && [ -f "$audit" ]; then
                echo "SKIP_COMPLETE family=$family scene=$scene q=$quota selector=$selector seed=$seed"
                continue
            fi
            "$script_dir/run_one.sh" \
                "$family" "$scene" "$selector" "$quota" "$seed" "$output"
        done
    done
done
