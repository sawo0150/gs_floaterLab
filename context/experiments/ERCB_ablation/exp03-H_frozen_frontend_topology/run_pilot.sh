...
root="$lab_root/results/ERCB_ablation/exp03-H_frozen_frontend_topology/utmm/square-1/q15"
...
if [ ! -f "$capture/psnr/online_final/final_result.json" ]; then
    "$script_dir/run_one.sh" record off rr_role_stratified "$capture"
fi
...
if [ ! -f "$topology_capture/psnr/online_final/final_result.json" ]; then
    "$script_dir/run_one.sh" replay record rr_role_stratified "$topology_capture"
fi
...
for selector in rr_role_stratified ercb_relative_floor_role_stratified; do
    ...
    "$script_dir/run_one.sh" replay replay "$selector" "$output"
done
