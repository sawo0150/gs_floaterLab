...
frontend_mode=${1:?usage: run_one.sh FRONTEND_MODE TOPOLOGY_MODE SELECTOR OUTPUT_DIR}
topology_mode=${2:?usage: run_one.sh FRONTEND_MODE TOPOLOGY_MODE SELECTOR OUTPUT_DIR}
selector=${3:?usage: run_one.sh FRONTEND_MODE TOPOLOGY_MODE SELECTOR OUTPUT_DIR}
output=${4:?usage: run_one.sh FRONTEND_MODE TOPOLOGY_MODE SELECTOR OUTPUT_DIR}
...
trace_root="$lab_root/results/ERCB_ablation/exp03-H_frozen_frontend_topology/utmm/square-1/q15"
...
export MAPPING_STEPS_PER_PACKET=15
export MAPPING_SEED=0
export MAPPING_DENSE_POSE_SOURCE=gt_absolute
export MAPPING_GT_TRAJECTORY="$lab_root/data/benchmarks/utmm/prepared/UTMM_Dataset/square-1/groundtruth.txt"
export MAPPING_GT_CAMERA_FRAME=utmm_robot
...
export MAPPING_CONFIG_OVERRIDE="$script_dir/config/utmm_trace.yaml"

exec "$common" utmm square-1 "$selector" "$output"
