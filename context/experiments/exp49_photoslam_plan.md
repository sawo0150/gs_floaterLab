...
### Phase A — 파이프라인 생존 검증 (수정 0줄)
목표: 빌드된 바이너리가 실제로 학습/렌더까지 도는지, GPU에서 안 죽는지 확인.
- A1: `train_colmap`을 우리가 이미 가진 COLMAP 씬(`data/03_rgb_3dgs_full` 또는 exp48의 `05_incremental_dense/chunk_000`)으로 실행 → GaussianMapper 배치 학습이 완주하는지.
- A2: (선택) `euroc_stereo`를 EuRoC 표본 시퀀스로 실행 → SLAM+매핑 전체 루프 생존 확인(흑백/fisheye라 품질은 목표 아님, 배관 검증용).
- 성공기준: 크래시 없이 완주 + `view_result`로 렌더 육안 정상.
...
### Phase C — Incremental replay 진입점 (진짜 기여)
목표: keyframe을 **시간 순서대로** GaussianMapper에 공급하는 스트리밍 진입점 작성.
- `train_colmap`(배치 일괄 로드)을 개조 → OpenMAVIS keyframe 순서대로 `handleNewKeyframe`-류 호출 + 각 keyframe 도착 시점에 그 시점까지의 정보만 사용(causal, exp48 원칙 유지).
- Photo-SLAM의 `useOneRandomSlidingWindowKeyframe`/times-of-use 메커니즘이 옛 영역을 어떻게 유지하는지 그대로 활용.
- 평가: Phase B와 동일 하네스. **목표 — incremental이 배치 상한에 얼마나 근접하는지**(exp48의 18dB 천장을 넘는 게 1차 관문).
