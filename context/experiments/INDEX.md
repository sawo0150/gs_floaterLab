...
| exp57 | 1차 목표를 **strict held-out 27dB**로 조정(MPS 절대 금지, RGB+IMU only, fixed 1.5×, tail 0). strict 최고 **23.982dB**. overlap-aware snapshot은 full paired에서 −0.845dB로 기각. 이어 regular `map()`의 총 step/schedule을 고정하고 마지막 1/7 iteration을 dense RGB 12장으로 교체했지만 600-frame control **22.461dB** 대비 weight 1.0/0.25가 **22.012/22.035dB**로 모두 하락해 dense-only replacement도 종료. 다음은 regular RGBD gradient를 보존한 dense-gradient projection/norm budget. 27dB 전 hard carve 보류 | **strict 최고 23.982 / 27dB 진행중** | [exp57](exp57_causal_background_polishing_plan.md) |
