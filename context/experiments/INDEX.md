...
| exp57 | 1차 목표 **strict-disjoint held-out 27dB**(MPS 금지, RGB+IMU only, fixed 1.5×, tail 0). fixed evaluator 252장을 모든 mapping supervision에서 제외. freeze 뒤 exact-newborn RGBD 1-step은 26.332/26.360을 반복했지만 paired refine=0가 **26.450dB**로 더 높아 기각; noisy online depth로 newborn geometry를 더 움직이지 않는다. 기존 freeze1050+append-only PPM+causal dense recipe의 유효 단일 최고를 26.450으로 갱신하되 동일 recipe 분산이 26.069~26.450이라 27dB는 반복 재현 필수. recent sampling/topology/final endpoint 종료, 27dB 전 hard carve 보류 | **strict-disjoint 26.450dB / 진행중** | [exp57](exp57_causal_background_polishing_plan.md) |
