...
| exp57 | 1차 목표를 **strict held-out 27dB**로 조정(MPS 절대 금지, RGB+IMU only, fixed 1.5×, tail 0). strict 최고 **23.982dB**. snapshot merge 기각. Dense foreground는 endpoint pose-confidence gate와 교체 slot 3→1로 control 손실을 −0.535→−0.140dB까지 회복했지만 순이득은 없어 기각. 다음은 dense photometric pose alignment 또는 overlap-aware spatial submap. 27dB 전 hard carve/floater 최적화 보류 | **strict 최고 23.982 / 27dB 진행중** | [exp57](exp57_causal_background_polishing_plan.md) |
