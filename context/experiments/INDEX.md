...
| exp57 | causal stable-map polishing + dense RGB. 4-offset 954장 고정-map 15k는 held-out/keyframe **30.389/30.321dB**지만 총 **2.124× live**. Aria **photo+IMU only** strict 1.5×(MPS 거부, tail 0)은 rolling **23.870/24.300dB**. 최초 freeze 18.004dB는 cutoff 단위 버그로 무효. 수정 재실험도 **22.002dB**, prune+online carve 결합은 **21.868dB**로 기각. CUDA-native subset은 수치 검증을 통과했지만 +2.05%뿐, fixed-map fast loop도 5k **19.30s / 28.325dB**라 30dB pure-online에는 update/chunk 구조 압축이 필요 | **품질 상한 성공 / pure-online rolling·same-tensor freeze 실패** | [exp57](exp57_causal_background_polishing_plan.md) |
