...
| exp57 | 1차 목표 **strict held-out 27dB**(MPS 금지, RGB+IMU only, fixed 1.5×, tail 0). per-view 진단으로 late coverage 붕괴 확정. append-only PPM birth는 paired +0.190dB. **정정: force-final 26.426/26.321은 evaluator가 항상 포함하는 마지막 RGB를 학습해 held-out 무효.** 기존 `mean_psnr`도 fixed eval+keyframe union임을 발견해 JSON에 고정 252-view metric/overlap을 추가, mapping-side disjoint guard 재검증이 다음. 기존 legacy 최고/반복 26.396/26.083. 27dB 전 hard carve 보류 | **legacy union 최고 26.396 / strict-disjoint 재검증중** | [exp57](exp57_causal_background_polishing_plan.md) |
