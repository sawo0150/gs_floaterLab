...
| exp57 | causal background polishing + dense RGB supervision. 고정 checkpoint keyframe-only full 5k는 held-out 23.47→25.62(+2.15), dense non-eval RGB 231장을 섞으면 22.83→**26.13(+3.30)**으로 gradient 효율 +1.15dB. **1.5× causal stream**에서는 다음 keyframe 도착 후 과거 중간 frame만 SE(3) 보간해 등록: dense 108장+3,821 step, held-out/keyframe **24.23/24.50**(keyframe-only background 대비 +0.25/+0.13), tracking 97.33s는 deadline 내지만 mapper drain 포함 98.47s로 0.84% 초과. 방향 채택, 5k 품질 유지는 아직 실패 | **dense 방향 채택·1.5× 부분 성공** | [exp57](exp57_causal_background_polishing_plan.md) |
