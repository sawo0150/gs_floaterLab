...
| exp53 | Frontend Tracking(exp52에서 확정한 진짜 실시간 병목) 자체를 가볍게 만드는 트랙. **축A(`iters1`/`iters2`, 4/2→1/0) 실행 완료·채택**: 온라인 루프 98.94→78.41s(−20.7%), evo APE(Sim3)는 세 단계(2/1·1/1·1/0) 전부 1.59cm로 완전 고정(ORB 13cm 대비 8배 우위 유지) — 이 세션 단일 축 최대 레버. exp54 축1과 조합 시 **72.91s(1.12배)**까지 도달(최고 기록). 축B~E(motion_filter.thresh·frontend_window/radius/nms·correlation 해상도·커널 튜닝) 및 ①②③ dense correspondence 계측은 미착수 | 진행중 | [exp53](exp53_frontend_realtime_plan.md) |
| exp54 | GS Mapping 연산 시간 ablation(exp52의 "rasterize+backward+loss_compute=81.4%" 발견을 구체화). **축1~3 실행 완료**: ①`pcd_downsample`(64→128) 채택(−3.3%, 부작용 없음) ②`pcd_downsample_init`(32→64) 기각 — densify가 성긴 init을 보충 증식해 최종 gaussian 수가 오히려 늘고 시간도 +1.2%(축2·축6 상쇄 관계 발견) ③`map() iters`(10→5) 기각 — −1.5% 시간에 −0.46dB PSNR로 ROI 나쁨, **이 시점 tracking이 이미 91% 비중임을 규명(exp53 우선순위 근거)**. 축1+exp53 축A 조합 = **72.91s(1.12배)**, `pcd_downsample=256`까지는 tracking-bound라 무효(72.68s, PSNR만 −1.3dB 추가 손실, 기각). 축4~7(렌더 해상도·max_viewpoints·densify 공격성·PPM) 미착수 | 진행중 | [exp54](exp54_gsmapping_speed_ablation_plan.md) |
