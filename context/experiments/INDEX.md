...
| exp57 | 1차 목표를 **strict held-out 27dB**로 조정(MPS 절대 금지, RGB+IMU only, fixed 1.5×, tail 0). strict 최고 **23.982dB**. Dense endpoint/pose-only 계열 기각 후 overlap-aware joint-context snapshot은 400-frame +0.178dB, residual delta merge 600-frame +0.146dB의 국소 신호를 냈지만 authoritative full paired A/B에서 **23.357→22.512dB(−0.845), 98.675s로 deadline도 실패**해 independent snapshot family 종료. 다음은 opportunistic idle GPU work가 아닌 결정론적 frontier view-op 재배분/regular map() 내 dense gradient 융합. 27dB 전 hard carve 보류 | **strict 최고 23.982 / 27dB 진행중** | [exp57](exp57_causal_background_polishing_plan.md) |
