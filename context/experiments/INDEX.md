...
| exp55 | 내용-적응 per-frame gaussian 예산(Sobel↔PSNR-이득 상관 실측 r=0.538) + carve loss 이식. **Phase 1+2+3 전부 실행·채택**: Phase1(캘리브레이션 2런)로 배율곡선(0.91~1.57x) 도출 → Phase2(베이스 128→256/init 32→64 + 내용-적응 배율 + per-keyframe 명시적 cap `enforce_kf_caps` 신규)로 **평균 gaussian −35.9%, 최종 −35.3%, PSNR/궤적 손실 없음(오히려 소폭 개선)** — 사용자 목표(평균 gaussian/frame 감소) 달성. Phase3(carve loss 온라인 근사, depth-violation 전용 신규 설계) — 기존 region GT가 1253/VIGS 좌표계에 적용 불가함을 확인 후 **carve_loss.py 자신의 검증된(AUC 0.98) 신호를 오프라인 진단 지표로 새로 구현**(`exp55_score_carve_vigs.py`, 신규 재사용 도구)해 직접 검증 — 가시 floater 수/비율/평균 score 네 지표 전부 일관 개선(−4~8%), PSNR·시간 비용 없음 → `carve_lambda=0.05` 채택. Phase 2Q는 미실행 | Phase1+2+3 완료 | [exp55](exp55_adaptive_density_carve_plan.md) |
