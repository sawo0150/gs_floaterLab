...
| exp57 | 1차 목표 **strict-disjoint held-out 27dB**(MPS 금지, RGB+IMU only, fixed 1.5×, tail 0). fixed evaluator 252장을 모든 mapping supervision에서 제외한 기준선 26.069dB. freeze 뒤 PPM newborn이 자신을 만든 RGBD keyframe을 한 번도 학습하지 않는 구조를 발견해 exact-newborn-row fresh-Adam 1-step을 추가, **26.332dB**, LPIPS 0.29921, 4,998 update, 75,068GS, 97.267s의 새 단일-run best. 단 상승 구간이 intervention 이전이라 scheduler 분산과 분리할 반복 검증 전 미채택. recent sampling/topology freeze/final endpoint는 종료, 27dB 전 hard carve 보류 | **strict-disjoint 26.332dB / 반복검증중** | [exp57](exp57_causal_background_polishing_plan.md) |
