...
| exp57 | 1차 목표 **strict held-out 27dB**(MPS 금지, RGB+IMU only, fixed 1.5×, tail 0). **strict best 24.319/24.385dB, 5,087 Gaussian-full random update, 97.264s로 deadline 0.386초 통과**. late map iters7→2 + all-Gaussian gradient + late global PGBA cutoff1120 채택. freeze1000은 6,370 update와 kf 26.015(+1.63dB)를 만들었지만 held-out 24.031로 late coverage 손실 때문에 기각. cutoff1070·tracking stride2·replay start400·CUDA batch2 기각. background LR×1.5는 무제한·step 제한·group 분리 모두 full 미달로 계열 종료. 27dB 전 hard carve 보류 | **strict 최고 24.319 / 27dB 진행중** | [exp57](exp57_causal_background_polishing_plan.md) |
