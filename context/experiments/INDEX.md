...
| exp51 | Incremental mapping 30dB+로: VIGS-SLAM 조사 결과 진짜 레버는 dense 프레임이 아니라 **depth supervision**(RGB만→RGB+SLAM보정 depth-pro depth loss)+keyframe 밀도(57→2/3/4배)+init 중복방지(빈 픽셀만 depth-lift). baseline D1-b 23.11 | 계획 | [exp51](exp51_dense_supervision_plan.md) |
