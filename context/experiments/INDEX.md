...
| exp57 | 실시간 품질 도약: frontier update와 causal background polishing을 분리하고, 동일 `n_global_views=6` 예산에서 residual·coverage·viewpoint novelty 기반 replay로 gradient 정보량을 높임. Phase 0에서 0~26k polishing 압축 곡선을 먼저 측정하고 SH/color-only부터 온라인 여유 시간에 분산 | **계획** | [exp57](exp57_causal_background_polishing_plan.md) |
