...
| Exp | 내용 | 상태 | 카드 |
|---|---|---|---|
| **VIGS ERCB ablation** | strict KF100 baseline 무재튜닝 전이는 +1dB **1/3**(`ego-centric-1` +3.899, `square-1` +0.361, `slow-straight-2` map 0). 첫 전역 수정인 provisional pre-IMU mapping은 `slow-straight-2`를 Adam 2,756/fixed 22.834/shared **+3.721dB**로 구조 복구 | **후보 checkpoint — untouched scene 전이와 loop coverage 개선 필요** | [running log](VIGS_ERCB_ablation/README.md) |
