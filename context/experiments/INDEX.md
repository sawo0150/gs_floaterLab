...
| exp57 | causal stable-map polishing + dense RGB. 4-offset 954장 고정-map 15k는 held-out/keyframe **30.389/30.321dB**지만 총 **2.124× live**. Aria **photo+IMU only** strict 1.5×(MPS 절대 금지, tail 0)은 rolling **23.870/24.300dB**. cutoff 수정 freeze 22.002, prune+online carve 21.868로 기각. independent snapshot도 appearance-only **23.551dB/1.596×**로 기각. frontier fast loop는 update +44.9%(2,088)에도 **23.728dB/1.585×**로 악화. 단순 update 증량 대신 arrived RGB의 coverage/residual/novelty 선택이 다음 | **품질 상한 성공 / pure-online 구조·update 증량 실패** | [exp57](exp57_causal_background_polishing_plan.md) |
