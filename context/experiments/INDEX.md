...
| exp55 | 내용-적응 per-frame gaussian 예산(Sobel↔PSNR-이득 상관 실측 r=0.538, exp44 Instant-GI/PPM 선례 확장) + carve loss 이식. **Phase 1+2 실행·채택**: 캘리브레이션 2런(dense64/sparse384)으로 내용 배율(0.91~1.57배) 곡선 도출 → 베이스를 128→256(+init 32→64)로 훨씬 성기게 하고 그 위에 내용-적응 배율 + per-keyframe 명시적 상한(`enforce_kf_caps`, growth_allowance=2.0, 신규 구현) 적용 → **평균 gaussian 수 −35.9%, 최종 개수 −34~35%, PSNR 동급(kf +0.16dB), evo APE Sim3 오히려 개선(1.90cm)** — 사용자가 이 라운드에서 명시한 목표(평균 gaussian/frame 감소) 달성, 시간은 −3.2%뿐(tracking-bound라 gaussian 절감이 wall-clock엔 부분 반영). **Phase 3(carve loss 온라인 근사, depth-violation 전용 경량화)도 구현·실행 완료**(크래시 없음, PSNR −0.1~0.3dB 비용) — floater 개선 여부를 잴 지표가 없어 기본 off 유지. Phase 2Q(품질 지향)는 미실행 | Phase1+2 완료 | [exp55](exp55_adaptive_density_carve_plan.md) |
