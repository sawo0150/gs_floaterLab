...
| exp55 | 사용자 제안: per-frame gaussian 예산을 내용-적응적으로(GT 이미지 Sobel/std ↔ 필요 gaussian 개수 관계, exp44 Instant-GI/PPM 선례 재사용) 배정 + 작은 init·densify로 채움 + carve loss(exp38~44d2)로 floater 통제. exp54 축2/6+2 결과("전역적으로 성기게+보정 증식 억제"는 시간 절감 없음)와의 차이를 먼저 명시(균일 축소가 아니라 "같은 총량의 효율적 재배분"이 목표). **1순위는 속도보다 pure_online PSNR 개선**(사용자 명시, 2026-07-22 추가) — Phase 2Q 신설: Q1(총 예산 상향, `pcd_downsample` 128 밑으로) Q2(PPM 가중치 강도 γ 스윕) Q3(init 전용 예산 별도 상향, axis2/6+2와 반대 방향) Q4(`densify_grad_threshold` 하향). Phase 1(캘리브레이션)·Phase 2(속도 지향 재배분)·Phase 3(carve loss 이식, CLAUDE.md 로드맵 2단계 합류 — VIGS 온라인 루프는 미래 keyframe을 모르므로 carve_loss.py의 배치 구조를 그대로 못 씀, 설계 질문부터 필요) | 계획 | [exp55](exp55_adaptive_density_carve_plan.md) |
