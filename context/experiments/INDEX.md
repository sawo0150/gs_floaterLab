...
| exp53 | Frontend Tracking(exp52에서 확정한 진짜 실시간 병목) 자체를 가볍게 만드는 트랙. dense correspondence 품질 계측(①confidence weight ②iteration별 delta 수렴 ③BA 재투영 오차)을 baseline으로 먼저 확립한 뒤, 축A(`iters1`/`iters2` 반복 축소, 최우선)·축B(`motion_filter.thresh` 상향, keyframe 밀도 억제)·축C(`frontend_window`/`radius`/`nms` 축소)·축D(correlation 해상도 축소)·축E(커널 튜닝, 보류) 순으로 탐색. Pass 기준: frontend 총합≤실제 녹화시간, evo APE(Sim3)는 ORB(13cm)보다 항상 우위 유지. exp52와 역할분담(exp52=mapping 실시간성 계속 진행, exp53=frontend 전담) | 계획 | [exp53](exp53_frontend_realtime_plan.md) |
| exp54 | GS Mapping 연산 시간 ablation(exp52의 "rasterize+backward+loss_compute=81.4%" 발견을 구체화). 7축: ①keyframe gaussian 밀도(`pcd_downsample`) ②init gaussian 밀도(`pcd_downsample_init`, 시퀀스 전체에 compounding하는 별도 축) ③`iters`(map() 반복횟수, 4개 단계에 곱으로 걸림) ④렌더 해상도 ⑤`max_viewpoints` ⑥densify 공격성 ⑦**PPM 기반(Sobel content-adaptive) 샘플링** — VIGS의 uniform random downsample을 exp44/48에서 검증된 PPM(`build_depthmono_ppm_chunks.py`)으로 교체, 같은 gaussian 예산으로 더 높은 PSNR 또는 동일 PSNR을 더 적은 개수로(=rasterize/backward 무료 절감) 달성 가능한지 검증. exp52 계측 인프라(map() 5단계+process_track_data 6단계) 재사용, "시간 대 PSNR" 트레이드오프 곡선이 목적(절대 pass 기준 없음) | 계획 | [exp54](exp54_gsmapping_speed_ablation_plan.md) |
