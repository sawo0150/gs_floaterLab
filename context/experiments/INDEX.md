...
| exp57 | 1차 목표 **strict-disjoint held-out 27dB**(MPS 금지, RGB+IMU only, fixed 1.5×, tail 0). RNG+shuffled+guard0ms 위 late-iters3가 **27.004/26.949dB**(평균 26.977), iters2 평균 대비 +0.219dB로 잠정 채택. 두 run 모두 5,161 update, 97.234~97.254s, tail0. 최초 27 초과는 나왔지만 반복 run이 27 미만이라 달성 확정 전 late-iters4 검증 | **strict-disjoint 27.004dB 단일 최고 / 반복 검증 진행중** | [exp57](exp57_causal_background_polishing_plan.md) |
