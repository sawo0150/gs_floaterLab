...
| exp57 | causal background polishing. 고정 checkpoint full 5k/36.3s는 held-out 23.47→25.62(+2.15), keyframe 23.91→27.94(+4.03); 26k는 keyframe 30.62 정상 재현(고정 held-out trajectory 때문에 후반 과적합 단정은 철회). SH-only scheduler는 1×에서 무효. **Aria 입력 1.5× slowdown(97.65s) A/B**: camera-pose background는 cache/autograd 충돌로 실패, 안전한 Gaussian-full은 3,194 step을 추가 지연 없이 흡수해 held-out/keyframe +0.25/+0.19dB·LPIPS 개선. 단 5k 품질 25.62는 유지 못했고 control/background 모두 약 102s로 deadline 4.4% 초과 — 다음은 2.5~3k deadline token gate | **Phase 0 완료·1.5× 부분 성공** | [exp57](exp57_causal_background_polishing_plan.md) |
