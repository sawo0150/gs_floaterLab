...
1. exp37 방식(dense confidence+monodepth init, plateau 없음)을 MPS 트랙에도 적용 — |Z|>4m=0·ray-density 우수함이 ORB 트랙만의 우연인지 재현 가능한 효과인지 확인.
2. exp37(SLAM core seed) vs 고confidence-seed dense init(5cm/60k판) 직접 비교 — plateau 없이 seed 종류만 바꿔 dense init 자체의 최적 seed 탐색.
3. ray-density 지표를 앞으로의 모든 신규 실험 평가에 표준 적용.
