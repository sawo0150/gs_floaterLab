# Round 8 요약 — Plateau의 사망 진단과 Carve Loss의 탄생 (2026-07-11, CPU 분석)

> 학습(GPU) 없이 순수 분석만으로 진행한 하루치 실험의 종합 요약.
> 상세 로그는 [round8_carve_loss_design.md](round8_carve_loss_design.md) (Round 1~10), [exp32_lineage_diag.md](../experiments/exp32_lineage_diag.md) §3 참조.
...
**성공 기준은 결과를 보기 전에 카드에 사전 등록해 둠** (round8_carve_loss_design.md "사전 등록" 섹션):
렌더 A/B는 A_safe 하락 ≤0.05dB, exp38은 PSNR ≥32.7 + 먼지 부하 절반 + 예산 준수.

## 7. 학습 검증 최종 결과 (2026-07-12 심야 자율 루프 — 이 장이 최종 결론)

> 상세 수치·전체 run 목록: [exp38_40_carve_track.md](../experiments/exp38_40_carve_track.md), 밤샘 실행 로그: [round8_gpu_queue_plan.md](round8_gpu_queue_plan.md)

### 채택 레시피 (exp40b) — 16회 학습으로 검증·재현 완료

`softlite opacity 압력(λ0.02) + 예산 prune(0.75%) + 출생 게이트(0.95) + carve-potential force(λ0.05)`
→ 실행: `--carve_loss_config configs/carve_loss/exp40b_force_soft.yaml`

| 트랙 | 가시 먼지 | PSNR |
|---|---:|---:|
| ORB (exp40b/재현) | 860 → **12 / 25** (-97~99%) | 32.58/32.45 — baseline 노이즈(±0.33dB) 경계 |
| MPS (exp39b) | 96 → **0** | **-0.10dB** |

### 비용의 전모 (held-out 2종 split + 지각 지표)

- held-out PSNR **-0.44dB** 실비용 존재 — 먼지가 글레어·노출 잔차를 실제 모델링하기 때문 (λ를 더 줄여도 안 사라지는 바닥).
- 그러나 **SSIM/LPIPS 동률** → 비지각적 비용. 사람 눈 기준으로는 "동등 품질 + 먼지 없음".
- **Pareto**: -0.1dB→먼지 -50%(prune+gate), -0.2dB→-70%(+force), -0.44dB→-90%+(softlite). 운영점 선택 가능.
- **train PSNR은 지표로 퇴출**: 사용자 수동 편집(시각적 정답)조차 train PSNR -3.7dB — floater는 지표 기생충.

### 메커니즘 (계측 수확)

- 진동 평형(coherence 0.26) → 약한 일관 3D force가 유효 — **plateau식 공간력의 부활 성공** (exp40a: 무비용 가시 먼지 -45%).
- split 출생의 29.5%가 허공, 부모는 저op 반투명 — "먼지가 먼지를 낳는" 연쇄. 게이트의 직접 근거.

## 7-구. 남은 리스크 (설계 시점 기록 — 대부분 위 검증으로 해소)
