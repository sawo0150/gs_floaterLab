...
- [시각자료] **실제 학습 체크포인트에서 동일 카메라 시점으로 직접 렌더링**(3dgs-custom render.py, 합성 아님): 305 baseline vs hybrid init(34.508→35.84dB, 먼지 461→4), 12F baseline vs hybrid+carve(32.034→35.07dB, 먼지 1289→243) — 두 장 다 육안으로도 선명도 차이가 뚜렷함(특히 12F는 fog로 흐릿하던 문·벽 디테일이 살아남)
- [발표 노트] "사용자의 원거리 통찰(먼지 98%가 원거리)은 진단으로는 맞았지만 처방(loss로 누르기)은 틀렸다 — 처방은 init이었다". 렌더 비교 자료는 render_cache/에 원본 보관, 재생성은 make_figures.py

### 슬라이드 5 — Init 레시피 상세: 어떤 anchor로 만들었나
- 슬라이드4의 "hybrid init"이 뭉뚱그려진 표현이 되지 않도록 — 실제로 어떤 재료·anchor로 만들어졌는지 정확히(`scripts/anchors/build_hybrid_init_scene.py`)
- **재료 3종 생성**:
  - **RoMA 삼각측량**: 인접 keyframe 쌍마다 dense correspondence 매칭(certainty>0.5) → 카메라 pose로 삼각측량 → **재투영 오차 <2px만 채택**(기하학적으로 검증된 점, frame당 최대 6,000쌍 샘플)
  - **Sobel-PPM depth-lift**: 이미지 gradient(Sobel) 크기에 비례한 확률맵으로 픽셀 4,000개/frame 샘플(텍스처 많은 곳에 조밀) → **SLAM 점으로 Huber 보정한 depth-pro**로 3D 역투영(스케일 보정 포함)
  - **원본 SLAM 점**: 항상 그대로 합류(회색 착색, 항상 신뢰)
- **병합**: 4cm voxel에서 dedupe, **RoMA가 우선권**(같은 voxel에 RoMA와 PPM 점이 겹치면 RoMA가 남음 — 기하 검증된 점을 우선)
- **재필터가 핵심 anchor**: 병합된 점들에 **exp43의 depth-anchor field**(depth-pro를 SLAM으로 보정해 만든 조밀 표면 anchor, depth 추론만 프레임당 ~0.5초)를 기준으로 free-space evidence `w = ρ(빈공간 증거비) · d5(anchor까지 거리)` 계산 → **w>0.5(빈 공간 의심)인 점을 init에 넣기 전에 미리 제거** — 이게 "먼지는 init에서 잡는다" 원칙의 실제 구현
- **왜 SLAM 점만으론 이 재필터가 안 되나**: exp43 교차 장면 진단 — 305/12F는 **SLAM 자기NN p50이 0.05m를 넘어**(1253은 0.042m) "SLAM 근접=표면" 가정이 깨짐(SLAM이 방을 성기게만 덮음) → 앵커를 SLAM 대신 depth-anchor(수십만~백만 점)로 바꿔야 진짜 표면 위치를 기준 삼을 수 있음
- **결과 규모**: 최종 hybrid init 점 수 — 305 **893,785개**, 12F **597,280개** (SLAM 원본은 각 수천~수만 개 수준에 불과)
- **carve 학습 단계의 anchor는 별개 선택**: 305는 학습에도 같은 depth-anchor field를 재사용(`exp43_305_depth.yaml`)했지만, 12F(축2b)는 SLAM 기반 "control" carve 설정(`exp43_12F_ctrl.yaml`, depth-anchor 아님)을 그대로 얹었는데도 −81% 청소에 성공 — **init 단계에서 이미 depth-anchor로 사전청소가 끝나 있어서, carve 쪽 anchor 정밀도의 중요성이 낮아진 것으로 보임**(가설, 추가 검증 필요)
- [시각자료] init 생성 파이프라인 다이어그램: RoMA/Sobel-PPM/SLAM 3재료 → 4cm voxel 병합(RoMA 우선) → depth-anchor 재필터(w>0.5 제거) → 최종 hybrid init
- [발표 노트] "init 개선"이라고만 하면 무슨 마법처럼 들릴 수 있어서 — anchor가 정확히 무엇이었는지(depth-anchor field, exp43에서 이미 만들어둔 자산) 짚고 넘어가는 슬라이드

### 슬라이드 6 — exp47: 속도 트랙 — 배치 트랙 실질 종료
...
- [시각자료] ① 시간-PSNR Pareto 산점도(8개 축, S2 초록 강조, TARGET 기각 빨강) ② **같은 카메라 시점 실제 렌더**로 "무손실" 직접 증명: 기준(66분, PSNR 35.07) vs S2(26.8분, 35.116) — 육안으로 차이를 못 찾을 정도로 동일
- [발표 노트] "배치 트랙은 이 지점에서 방법론적으로 완결됐다고 판단 — 남은 질문은 incremental로만 답할 수 있다". 오른쪽 렌더 비교가 "화질 무손실" 주장의 직접 증거

---

## §2. 왜 Incremental로 전환했나 (슬라이드 7–8)

### 슬라이드 7 — 최종 목표(North Star) 재확인
...
### 슬라이드 8 — 왜 순서가 "품질 먼저, floater는 그다음"인가
...
## §3. Incremental 여정 — 서사 (슬라이드 9–15)

### 슬라이드 9 — exp48: 자체구현의 실패, 18dB 천장
...
### 슬라이드 10 — exp49: Photo-SLAM 채택 — 아키텍처 전환만으로 +3.9dB
...
### 슬라이드 11 — exp50 (병행 트랙): 라이브 인프라 — 지금 PSNR엔 안 잡히지만 최종 시스템에 필수
...
### 슬라이드 12 — exp51 축A+B: depth supervision + init 중복방지 — 실측 레버
...
### 슬라이드 13 — exp51 축C+F: "더 넣으면 되겠지"가 틀렸다
...
### 슬라이드 14 — 자기수정: "배치 30.2dB가 상한"이라는 착각
...
### 슬라이드 15 — 시각 진단 확정: 잔여 갭의 정체는 depth-init 바늘형 floater
...
### 슬라이드 17 — 진행 중 걸림돌: 전체 프레임+vanilla급 예산 실험이 CUDA OOM
...
### 슬라이드 18 — 다음 스텝: 축E — 검증된 Carve Loss를 Incremental에 이식
