...
사용: python build_ppt.py  (같은 폴더의 ppt_outline_20260717.md 사용)
출력: gs_floater_deck_0717.pptx  → soffice로 PDF 변환은 셸에서 별도.

주의: img/<파일>.png는 아직 없음(신규 발표라 그림 미제작) — IMG_MAP은 비워둠.
그림을 만들면 슬라이드 번호에 맞춰 IMG_MAP에 항목을 추가할 것 (0714 폴더의 make_figures.py를 참고해도 됨).
"""
import re
import sys
...
MD = HERE / "ppt_outline_20260717.md"
OUT = HERE / "gs_floater_deck_0717.pptx"
IMG = HERE / "img"
...
def build():
    ...
    title_txt = "gs_floaterLab 진행상황 업데이트 — 07/14 → 07/17"
    sub_txt = "배치 트랙 마무리(305·12F) · 왜 Incremental로 전환했나 · Incremental 여정과 현재 갭 진단 · 다음 스텝"
