...
def build():
    s=slide(notes='00:00–00:10 | 발표자: 박상원. 표지 이름 순서: 박상원, 김동휘, 송채현. No speech script.')
    ...
    txt(s,.7,3.35,8.6,.52,'박상원, 김동휘, 송채현',23,center=True,font='Noto Sans CJK KR')
    ...
    s=slide('When Should We Add More Views?','Use frames between keyframes, paced by completed optimization.',
            'Method: Optimization-Guided View Growth. Diagram is conceptual.',
            '02:00–02:35 | More views provide evidence but dilute updates. Growth follows completed mapping updates. No hardware-invariance claim.')
    txt(s,.45,1.80,1.9,.50,'Tracking\nkeyframes',14)
    for i in range(9):
        key = i in (0,4,8)
        labeled_box(s,2.55+i*.74,1.86,.53,.65,'KF' if key else 'RGB',NAVY if key else LIGHT,WHITE if key else GRAY,11)
    txt(s,2.55,2.70,6.45,.36,'Intermediate RGB frames also carry mapping information.',14,center=True)
    labeled_box(s,.65,3.43,2.55,.72,'Completed updates',LIGHT,size=16)
    arrow(s,3.47,3.67,.48)
    labeled_box(s,4.20,3.43,2.2,.72,'Add a new view', 'FAEEDF',ORANGE,16)
    arrow(s,6.69,3.67,.48)
    labeled_box(s,7.39,3.43,1.96,.72,'Training set',LIGHT,size=16)
    ...
    s=slide('Which Views Should We Optimize?','Give under-trained observations more opportunities, while mixing views.',
            'ERCB: Entropy-Regularized Count Balancing. Illustrative counts, not experimental data.',
            '02:35–03:10 | Intuition only. Latest intro uses temporal-group correction; method draft also contains per-view derivation. No equality or convergence guarantee claimed.')
    txt(s,.5,1.73,4.0,.4,'Uneven optimization exposure',17,NAVY,center=True)
    base=3.80
    for i,(h,lbl) in enumerate([(1.30,'A'),(1.0,'B'),(.5,'C'),(.25,'D')]):
        x=.98+i*.83
        box(s,x,base-h,.46,h,NAVY if i<2 else ORANGE)
        txt(s,x-.05,base+.08,.56,.28,lbl,12,center=True)
    line(s,.70,base,4.18,base,GRAY)
    txt(s,.70,4.22,3.5,.27,'Observed temporal groups (schematic)',11,center=True)
    arrow(s,4.59,2.9,.52,.25)
    txt(s,5.43,1.78,3.87,.50,'Count-aware randomized selection',17,NAVY,center=True)
    for i,lbl in enumerate(['D','B','C','A']):
        labeled_box(s,5.62+i*.9,2.70,.63,.63,lbl,'FAEEDF' if lbl in ('D','C') else LIGHT,ORANGE if lbl in ('D','C') else NAVY,19)
    txt(s,5.45,3.60,3.93,.70,'Correct severe deficits\nPreserve sampling diversity',17,center=True)
    takeaway(s,'Balance learning opportunities without fixing a rigid order.')
    ...
    prs.core_properties.author='박상원, 김동휘, 송채현'
