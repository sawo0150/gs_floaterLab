#!/usr/bin/env python3
"""Run the current VIGS final-v7 online mapper on RPNG and UTMM."""
...
RUNNER = VIGS_REPO / "exp69_axes/run_decoupled_geometry.sh"
PYTHON_ENV = Path("/home/colin/miniconda3/envs/vigs-slam-5090")
PROTOCOL = "fixed1p5_prefinal_queue_drain_v1"
METHOD = "vigs_final_v7"
RUNSET = "2a3eeeb5_dirty_20260910"
...
def main() -> int:
    ...
    env["EXP69_REPLAY_TIME_SCALE"] = "1.5"
    ...
    for dataset, sequence, frames in selected:
        ...
        command = [str(RUNNER), f"benchmark_{dataset}_{sequence}", str(out), "v7"]
        manifest = {
            "dataset": dataset,
            "sequence": sequence,
            "frames": frames,
