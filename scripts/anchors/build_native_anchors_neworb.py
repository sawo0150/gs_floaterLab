#!/usr/bin/env python3
"""Native anchor generation for the rebuilt OpenMAVIS session (2026-07-07 run).

Same pipeline as scripts/diagnostic/render_completed_plateau_ellipsoids.py
(Stage1-3 SLAM filtering + monodepth-completed voxel seeding, v4 params),
but pointed at data/03_rgb_3dgs_full so anchors are generated *natively* in
that dataset's world -- no cross-session alignment transform needed.

Difference from the original script: data/03_rgb_3dgs_full/sparse/0/images.txt
has no stored 2D-3D correspondences (POINTS2D line empty), so instead of
reading points2d/point3d_ids we reproject the Stage-3 filtered SLAM points
directly into each keyframe camera to get (u, v, z_slam) pairs for the
monodepth scale/shift calibration.
"""
import json
import os
...
OUT_DIR = Path("/home/wosas/Desktop/Incremental_mapping_test/gs_floaterLab/results/diagnostic") / f"native_anchors_neworb_v4_{_TS}"
...
def load_and_filter_slam_points():
    print("Loading and filtering SLAM points (Stage 1-3)...")
    conf = {}
    for line in open(MAP_PTS_JSONL):
        d = json.loads(line)
        conf[d["map_point_id"]] = d["observations"]

    pts_list, obs_list, pid_list = [], [], []
    ...
    pts_stage3, obs_stage3 = pts[mask], obs[mask]
    print(f"  Stage 3 (kNN isolation, thr={threshold:.3f}m): {len(pts_stage3)}")
...
def main():
    ...
    pts_slam, obs_slam, cams_all, filt_lo, filt_hi = load_and_filter_slam_points()
    ...
    for image_id in kf_image_ids:
        ...
        t = img_data["tvec"]
        xyz_cam = (R @ pts_slam.T).T + t  # world -> camera
        z = xyz_cam[:, 2]
        front = z > 0.1
        ...
        ok = z_mono > 0
        if ok.sum() < 2:
            print(f"  [skip] {img_data['name']}: only {ok.sum()} SLAM correspondences")
            continue
        ...
        print(f"  {img_data['name']}: {ok.sum()} correspondences, calibrated")
