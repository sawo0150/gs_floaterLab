#!/usr/bin/env python3
"""Dense init point cloud generation (separate from anchor generation).

Anchors (build_native_anchors_neworb*.py) are deliberately SPARSE -- they're
potential-field seeds for the plateau loss, spaced >=0.5m apart on purpose.
Using them directly as init (exp27/31/34) caps Gaussian capacity and costs
~1.4dB vs exp08's 626k-point MPS init.

This script builds a genuinely DENSE init instead:
  - SLAM core: Stage1-3 filtered points (obs>=3, camera-bound, kNN-isolation
    cleaned) -- same set as the "plain" native anchor's SLAM part. Kept in
    full to maximize real-geometry coverage.
  - Depth calibration: monodepth scale/shift fit uses ONLY high-confidence
    correspondences (obs>=10 & found_ratio>=0.5), same principle as the
    high-confidence anchor variant -- avoids letting noisy low-confidence
    matches skew the per-keyframe calibration.
  - Voxel seeding: much finer voxel size (5cm vs anchor's 30cm) and denser
    pixel stride (4px vs 16px), and the min-spacing constraint (D_TARGET)
    is dropped (set == voxel_size) since density, not sparse potential-field
    spacing, is the goal here.
  - Multi-view consistency (MIN_HITS=2) is kept to reject single-frame
    monodepth noise.

Output is a full 3DGS scene (images/ symlink + sparse/0/{cameras,images,
points3D}.txt) ready to train on directly, at
results/datasets/orb_dense_confmono_init_scene.
"""
import json
import os
import shutil
...
OUT_DIR = Path("/home/wosas/Desktop/Incremental_mapping_test/gs_floaterLab/results/diagnostic") / f"dense_init_neworb_{_TS}"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# SLAM core filtering (matches the "plain" native anchor -- broad coverage)
KNN_K = 5
KNN_ISO_MULT = 3.0
CORE_OBS_MIN = 3

# Depth calibration fit uses high-confidence correspondences only
FIT_OBS_MIN = 10
FIT_FOUND_RATIO_MIN = 0.5

# Dense voxel seeding (vs anchor's VOXEL_SIZE=0.30, STRIDE=16, D_TARGET=0.50)
VOXEL_SIZE = 0.05
STRIDE = 4
MIN_HITS = 2
...
def stage3_filter(pts, obs_mask, lo, hi):
    """Camera-bound + obs_mask + kNN isolation, returns filtered pts."""
    valid = ((pts >= lo) & (pts <= hi)).all(axis=1) & obs_mask
    pts = pts[valid]
    if len(pts) <= KNN_K + 1:
        return pts
...
def check_distance_constraint(pos, G_occ, voxel_size, d_target):
    if d_target <= voxel_size:
        key = tuple(np.floor(pos / voxel_size).astype(int))
        return key not in G_occ
    key = tuple(np.floor(pos / voxel_size).astype(int))
    for dx in (-1, 0, 1):
        for dy in (-1, 0, 1):
            for dz in (-1, 0, 1):
...
def main():
    ...
    print(f"raw SLAM points: {len(pts_all_raw)}")

    core_mask = obs_arr >= CORE_OBS_MIN
    pts_core = stage3_filter(pts_all_raw, core_mask, lo, hi)
    print(f"SLAM core (obs>={CORE_OBS_MIN}, camera-bound, kNN-isolation): {len(pts_core)}")

    fit_mask = (obs_arr >= FIT_OBS_MIN) & (fr_arr >= FIT_FOUND_RATIO_MIN)
    pts_fit = stage3_filter(pts_all_raw, fit_mask, lo, hi)
    ...
    print(f"Keyframe RGB images: {len(kf_image_ids)}")
    ...
    # Dense voxel seeding, G_occ seeded from the broad SLAM core
    G_occ = {}
    for pt in pts_core:
        key = tuple(np.floor(pt / VOXEL_SIZE).astype(int))
        G_occ.setdefault(key, pt)
    n_core_voxels = len(G_occ)
    ...
    pts_core32 = pts_core.astype(np.float32)
    pts_all = np.vstack([pts_core32, pts_virtual]) if len(pts_virtual) else pts_core32
    print(f"\nSLAM core: {len(pts_core32)} (occupied {n_core_voxels} voxels)")
    print(f"Dense virtual: {len(pts_virtual)}")
    print(f"Total dense init: {len(pts_all)}")

    np.save(OUT_DIR / "init_slam_core.npy", pts_core32)
    np.save(OUT_DIR / "init_virtual_dense.npy", pts_virtual)
    np.save(OUT_DIR / "init_all.npy", pts_all)
    core_rgb = np.tile([59, 130, 246], (len(pts_core32), 1))
    virt_rgb = np.tile([240, 120, 30], (len(pts_virtual), 1))
    all_rgb = np.vstack([core_rgb, virt_rgb]) if len(pts_virtual) else core_rgb
    _write_ply(OUT_DIR / "init_all.ply", pts_all, all_rgb)

    meta = {
        "model": "depth_pro",
        "purpose": "dense_init (not anchor)",
        "core_obs_min": CORE_OBS_MIN,
        ...
        "min_hits": MIN_HITS,
        "d_target_m": D_TARGET,
        "n_slam_core": int(len(pts_core32)),
    ...
    SCENE = Path("/home/wosas/Desktop/Incremental_mapping_test/gs_floaterLab/results/datasets/orb_dense_confmono_init_scene")
    ...
    with open(SCENE / "sparse/0/points3D.txt", "w") as f:
        ...
        f.write(f"# Number of points: {len(pts_all)}, source: dense_init_neworb "
                f"(SLAM core obs>={CORE_OBS_MIN}: {len(pts_core32)} + dense monodepth virtual "
