...
outdir="$RESULTS/exp43rot2_static_30k_${ts}"
mkdir -p "$outdir"; cd "$REPO"
# rot 장면 전용 config (points_txt 미지정 → 자기 points3D 사용, dynamic carve 포함)
sed 's|^points_txt:.*||' configs/carve_loss/exp45b_dynamic.yaml > configs/carve_loss/exp43rot_static.yaml
conda run -n 3dgs python train.py \
    ...
    2>&1 | tee "$RESULTS/exp43rot2_static_30k_${ts}.log" | tail -1
echo DONE
