...
outdir="$RESULTS/exp45b_dynamic_carve_30k_${ts}"
...
conda run -n 3dgs python train.py \
    ...
    --carve_loss_config "$REPO/configs/carve_loss/exp45b_dynamic.yaml" \
    2>&1 | tee "$RESULTS/exp45b_dynamic_carve_30k_${ts}.log" | tail -1
echo DONE
