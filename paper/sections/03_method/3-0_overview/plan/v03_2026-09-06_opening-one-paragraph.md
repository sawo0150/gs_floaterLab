...
> **번호 없는 도입. roadmap 한 문단, 그게 전부다.** 6문장 16줄, 0.15 p.
...
| 3 | ③④ §3.1 | **고정 집합 위의 고정 예산이 성립하지 않으므로** 완료된 GPU service 가 증가 속도를 정한다 → **which makes** 그 속도가 집합 크기·하드웨어와 무관해진다 |
| 4 | ③④ §3.2 | **셔플링은 유한 고정합 위에 정의되는데** 자라는 집합은 그렇지 않으므로 count 균형 비복원 block → **which** 무작위성을 버리지 않고 노출을 고르게 한다 |
| 5 | ③④ §3.3 | **기하는 가진 뷰로 결정되는데** free space 는 아직 미결정이고 **floater 가 그 미결정의 가시화**이므로 causal carve evidence → **which** 미래 프레임 없이 제거한다 |
| 6 | ⑤ 그림 | Fig. 2 가 **한 원인의 세 결과**가 한 pass 안에서 맞물리는 것을 보여준다 |
