...
% ★ 2026-09-16: greeking → 실제 문장. 7문장 그대로, 계획된 자리도 그대로다.
%   1 전제 / 2 대비 / 3 셋을 평이한 말로 세어줌 / 4·5·6 같은 꼴 / 7 그림 지목
%   4·5·6 의 꼴: [깨지는 offline 가정] → [무엇을 하는가, \S ref] → ["which …" 얻는 것]
%   'which …' 절은 lmrs §4 도입의 thereby / which results in / which eliminates 자리다.
% ⚠ claims A1 제약: 이 분해가 **유일하거나 최적**이라고 말하지 않는다.
%   그래서 "the three decisions that determine …" 가 아니라 "three points … where"로 쓴다.
\begin{figure}[t]
  \centering
  \fcolorbox{slotgray}{slotbg}{\parbox[c][3.4cm][c]{0.92\linewidth}{\centering
    {\sffamily\scriptsize\bfseries\color{skelblue}FIG.\,2 \textemdash\ SYSTEM DIAGRAM}\\[3pt]
    {\sffamily\scriptsize\color{slotgray}one pass of the mapper\\
     completed service $\rightarrow$ credit $\rightarrow$ quota $\rightarrow$ admitted views
     $\rightarrow$ block order\\
     three decisions drawn as three separate boxes \quad\textbullet\quad claim A1}}}
  \caption{One pass of the mapper. The three consequences of a single cause enter at three
           separate points, and nothing routes them through a shared keyframe test.}
  \label{fig:system}
\end{figure}

Our goal is a map that converges quickly --- in photometric quality and in geometry alike ---
under the compute that a live stream leaves available.
Instead of letting a single keyframe rule decide how supervision reaches the map, we set three
decisions separately, each from what the optimizer has actually completed
(Fig.~\ref{fig:system}).
We first let completed GPU service, rather than the keyframe rule, set how fast the training view
set grows, which makes that rate independent of the size of the set (\S\ref{ssec:growth}).
Then, we draw each block of views from a count-balanced distribution obtained from an
entropy-regularized objective, which evens out exposure across the trajectory without giving up
random order (\S\ref{ssec:ercb}).
Finally, we constrain the free space that photometric loss alone leaves undetermined, using carve
evidence taken only from frames already seen (\S\ref{ssec:carve}).
\pend{sentence 5 assumes carving addresses \emph{under-observation} floaters --- teammate}
...
\slot{P3 \textemdash\ DERIVING (relaxation 2) + EQUALITY AUDIT}{20}{%
  \textbf{Put (a) back as a term, not as a patch.} Eq.~(\ref{eq:objective}): the first term is
  (b), the second is (a), and $\tau$ is the knob P1 asked for --- $\tau\!\to\!0$ recovers the
  deterministic rule, $\tau\!\to\!\infty$ recovers uniform. \textbf{At a given step the counts are
  fixed}, so the objective is linear in $p$ plus $\tau$ times negative entropy: strictly convex on
  the simplex, hence a \emph{unique} minimizer of Gibbs form \pend{cite --- standard; candidates
  in refs/10\_maxent\_control}. \textbf{The step that is ours is the cancellation}: $\Delta_i\Phi$
  differs from $n_i$ only by terms common to every $i$, which drop out in the normalization,
  leaving Eq.~(\ref{eq:gibbs}) with $\beta=1/\tau$. \emph{The Lagrangian algebra is standard and
  goes to the supplement} --- say so in half a clause, the way chen2026cover writes ``automatic by
  Rayleigh quotients''.
...
\begin{equation}\label{eq:objective}
  \min_{p \in \Delta}\;\; \underbrace{\sum_i p_i\,\Delta_i\Phi}_{\text{(b) balance}}
  \;-\; \tau \underbrace{H(p)}_{\text{(a) mixing}}
\end{equation}
...
\slot{P4 \textemdash\ EXTENSION (relaxation 3) + WHEN IT IS EXACT}{22}{%
  ...
  Gumbel-Top-$k$, which is how it is implemented). No display for it --- name and cite, as with
  the Gibbs form. That form is also what makes the audit in P3 work: with equal weights a
  Plackett--Luce draw \emph{is} a uniform permutation.
  Applying Eq.~(\ref{eq:gibbs}) exactly would mean recomputing $p$ after every single selection;
  we instead freeze the counts at the start of a block, $n_i^{(b)}$, and draw all $K$ under that.
  \pend{DO NOT call this a first-order approximation until checked. Drawn views leave the
  candidate set, so their count updates cannot change the relative weights of the remaining
  candidates, and the common terms cancel anyway --- under the model as written the frozen draw
  looks \textbf{exact}, which is a stronger statement than the one we were making. It could still
  break if $K$ exceeds the candidate count and wraps, or if $n_i$ increments on \emph{use} rather
  than on selection. Verify against exp72 code, then state the condition.}
  How much randomness survives is measured by $\rho_H$, defined with the metrics in
  \S\ref{sec:exp}.}

\slot{P5 \textemdash\ WHERE THE PROXY AND THE GOAL PART WAYS}{16}{%
  ...
  $+0.362$\,dB; on another it was $0.971$, close to even. Larger $\beta$ recovered only to $0.605$
  and cost entropy. \textbf{We therefore do not claim this family improves lifetime-count equality
  in general.}
  \textbf{Offer a hypothesis and label it as one}: balancing raw counts pulls late-admitted views
  up, but a view admitted very late has few updates left to be pulled up \emph{in}, so on a
  trajectory whose content is back-loaded that shortfall can dominate --- \emph{we have not tested
  this}. Saying we lack a verified explanation is better than inventing one.
  \textbf{Then tell the reader what it means}: if lifetime equality is itself the objective, this
  is not the tool for it (\S\ref{sec:exp}).}
