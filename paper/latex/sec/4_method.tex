...
%     §3.2  90줄 0.82p   문단 84 (12/12/24/24/12) + 수식2 6
...
\slot{P1 \textemdash\ ROADMAP + THE EXACT OBJECT}{12}{%
  The object we would like to optimize is the final selection-count variance over the whole run,
  chosen against an arrival process that is not yet known --- a combinatorial problem over an
  unknown horizon, not solvable online. \textbf{Say that first.} The derivation then relaxes it in
  three steps: (1) a one-step greedy on a potential $\Phi$; (2) an entropy-regularized form of
  that step, which has a unique closed-form solution; (3) a $K$-view block that freezes the counts
  while it draws. Full algebra in the supplement.}

\slot{P2 \textemdash\ FORMULATING (relaxation 1)}{12}{%
  Let $n_i$ be how many times view $i$ has been selected. Define the potential
  Eq.~(\ref{eq:phi}), a monotone function of the count variance. Selecting $i$ raises $n_i$ by
  one, and the resulting increase is $\Delta_i\Phi = n_i - \bar n + \tfrac12(1-\tfrac1N)$
  \emph{(inline, not displayed)}. The last term does not depend on $i$, so the greedy step reduces
  to \textbf{picking the least-selected view} --- which is deterministic, and a deterministic order
  is exactly the randomness that shuffling was there to provide. That tension is what the next
  step resolves.}
...
\slot{P3 \textemdash\ DERIVING (relaxation 2) + EQUALITY AUDIT}{24}{%
  Rather than take the deterministic $\arg\min$, minimize its expectation under a distribution $p$
  with an entropy penalty: $\min_p \sum_i p_i\,\Delta_i\Phi - \tau H(p)$ on the simplex. State what
...
\slot{P4 \textemdash\ EXTENSION (relaxation 3) + ASSUMPTION LEDGER}{24}{%
...
\slot{P5 \textemdash\ IN-METHOD FAILURE}{12}{%
  % lmrs §4.2 형식 — 자기 설계의 한계를 본문에서, 표 forward pointer 와 함께
  The potential we chose equalizes \emph{selection counts}, not \emph{lifetime exposure}: a view
  admitted late has had fewer chances, and equalizing $n_i$ does not correct for that. Measured,
  the middle-to-first exposure ratio falls from $0.758$ to $0.520$ on one trajectory
  (\S\ref{sec:exp}). Counts alone as the potential is a design choice, and this is its limit.}
