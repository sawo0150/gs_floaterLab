...
\slot{METRICS}{10}{%
  ...
  within budget (Taming3DGS's \texttt{Peak \#G}). \emph{Scheduling}: report the workload-ledger
  regime ($S<U$ scarce or $S\ge U$ rich), completed dense services, unique eligible views, and
  the worst-quartile deltas defined from the RR control. Name the tools (\texttt{evo},
  \texttt{torchmetrics}) the way cartgs does, with footnote URLs. All numbers are
...
\slot{IMPLEMENTATION DETAILS}{9}{%
  ...
  what changed} --- $\kappa$, $K$, $\rho$, and $\gamma$ --- which is how cartgs writes it; these values
...
\caption{\skel{TABLE 4} Selector-only ablation under exact candidate membership and completed
  work.  Workload-ledger scarcity is assigned before inspecting image quality; rich sequences are
  reported as controls rather than used to demand an ERCB win.}
\label{tab:order}
\begin{tabular}{lll|ccc}
\hline
selector & $\rho$ & $\exp(\gamma)$ & PSNR$\uparrow$ & worst-Q1$\uparrow$ & RR-hard-Q1$\uparrow$ \\
\hline
RR                 & --            & --            & -- & -- & -- \\
ERCB, interval only& 0.75          & 1.5           & -- & -- & -- \\
ERCB, global residue& \textbf{0.75}& \textbf{1.5}  & -- & -- & -- \\
\hline
\end{tabular}
\end{table}

\noindent\textbf{Service-shortfall ERCB.}\;
\slot{ITEM 2 \textemdash\ REGIME FRAMING first, matches \S\ref{ssec:ercb}}{15}{%
  Assign service regime from the workload ledger before opening PSNR: $S<U$ is structurally
  scarce and $S\ge U$ is rich.  In rich C1 pools, report ERCB versus RR as a control without a
  mandatory win; both selectors may complete the same global coverage.  The primary module test
  disables C1, holds candidate membership and work exact, and reports sequence-equal overall and
  RR-hard-Q1 deltas on scarce sequences.  Table~\ref{tab:order} separates the old interval-local
  queue from the global-residue repair, so repeated service cannot be mistaken for coverage.
  Report every sequence, including negative results, and do not mix the Full-versus-vanilla gate
  with this selector-only ablation.}
