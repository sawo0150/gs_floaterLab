...
| 모양 | 논문 | 언제 쓰나 | 우리 |
|---|---|---|---|
| **A. 대칭형** — 문제 N개를 이름 붙여 나열하고 해법 N개가 같은 순서로 대응 | cartgs | 기여가 병렬적 부품 여러 개일 때 | ⚠ **11편 중 1편뿐.** 진단 소절은 두지 않기로 함 (2026-09-06) |
...
## 우리 §3 = C(접속) + B(§3.2 내부). A 는 채택하지 않음

⚠ **2026-09-06 정정.** 아래 §3.0 문단 계약(5문단)은 폐기됐다.
코퍼스 조사 결과 Method 도입은 5~7문장 8~13줄 **한 문단**이 관행이고,
진단에 소절을 준 논문은 11편 중 cartgs 하나뿐이었다.
→ 도입은 **roadmap 1문단 + `Notation.` run-in**, 진단은 각 소절 P1 으로 분산,
"세 결정으로 분해"는 §1 이 진다. 확정본은
